...
public class CPMStepPlot extends CPMXYPlotHandler
{
    ...
    {
        // Set the chart size based on the number of plots, which in turns sets the subplot size. A
        // value is subtracted from the width to make room for the vertical scroll bar (the actual
        // value used makes no difference so long as it's larger than what's needed to display the
        // scroll bar)
        ...
        // Adjust the gap between the subplots. 1 is subtracted so that a zero gap value actually
        // equates to -1, which is the value required to reduce the gap to its smallest value
        ...
        {
            // Turn on/off subplot y-axis tick labels based on the user- selectable flag
            plot.getRangeAxis(0).setTickLabelsVisible(plotPrefs.isStepPlotShowLabels(false));
        }

        // Enable the chart progress listener so that the spacing for the start/end/delta fields
        // and the x-axis scroll bar can be adjusted
        setChartProgressListenerEnable(true);
    }

    /**********************************************************************************************
     * Extend CombinedDomainXYPlot to override its panning methods so that the y-axis can't be
     * panned, and so that the label plot x-axis and scroll bar follow the combined plot's x-axis
     * panning
     *********************************************************************************************/
    private class CPMCombinedDomainXYPlot extends CombinedDomainXYPlot
    {
        ...
        /******************************************************************************************
         * Pass the domain (x-axis) panning event through to its normal handler, then adjust the
         * x-axis scroll bar
         *****************************************************************************************/
        @Override
                                  ...
                                  Point2D source)
        {
            ...
            // Adjust the x-axis scroll bar thumb size, position, and increments
            adjustScrollBarIncrements();
        }

        /******************************************************************************************
         * Override the normal range (y-axis) panning handler to prevent panning of the y-axis
         *****************************************************************************************/
        @Override
    ...
    protected JFreeChart createPlotChart()
    {
        ...
        for (int index = 0; index < getPlotSeries().size(); index++)
        {
            ...
            // Set the color to the ID color or the CPU activity line color, if this line is
            // present
            ...
            // Set the fill color by making the line color darker and the color transparent (the
            // outline color is used for filling)
    ...
    protected void convertLogDataToXYSeries()
    {
        ...
        // Set the flag indicating that the CPU activity line should not be shown
        ...
        for (PerformanceID pid : perfIDs.getPerfIDList())
        {
            ...
            if (pid.isPlotPerfID())
            {
                ...
                // Set plot's y-axis label (use the ID if no ID name exists) and the min/max tick
                // labels
                ...
                // Set up a temporary variable to shorten the subsequent method calls
                ...
                {
                    // Check if this is the first data point following a log data gap for the
                    // current ID
                    if (gapIndex < perfLog.getDataGapIndex().size()
                        && log.equals(perfLog.getPerfLogData().get(perfLog.getDataGapIndex().get(gapIndex))))
                    {
                        // Set the first point flag so that the next plotted point (is treated like
                        // an initial point
                        firstPoint = true;
                    }
                    ...
                    {
                        // Check if this is the first point for this ID or the first point
                        // following a gap between two logs
                        if (firstPoint)
                        {
                            // Reset the flag
                            firstPoint = false;

                            // Add a starting point at initial time stamp for the current log
                            // file's data. This is so that the plot line starts at the beginning
                            // of the x-axis for the first file and that the IDs' entry/exit
                            // statuses are reset following a data gap. It's assumed that an ID's
                            // initial state is 'entry' if its first logged point is 'exit', and
                            // vice versa
                ...
                {
                    // Add an end point at the maximum time stamp so that the plot line ends at the
                    // end of the x-axis. The ID value used is the same as its last recorded value
        ...
        // Add the CPU activity subplot if more than one series is selected for display (with only
        // a single series displayed, the CPU plot is identical to the displayed ID's plot), or if
        // no IDs are selected (so that a flat line can be drawn showing the CPU idle)
        if (seriesCount != 1)
        {
            // Create the CPU activity subplot line. This series checks the displayed IDs' at each
            // log entry and, if all are inactive (i.e., between an exit and entry event) then the
            // CPU is shown as idle; otherwise the CPU is shown as busy
            ...
            if (seriesCount != 0)
            {
                // Set the flag indicating that the CPU activity line should be shown
                hasActivityLine = true;

                // Of the IDs to be plotted, determine the number of those that are initially
                // active (i.e., the ID's first event is an exit)
                int activeIDCount = perfLog.getActiveIDCount(-1);

                // Add the start point. If any IDs are active at the beginning then start with the
                // CPU showing busy; otherwise assume the CPU is idle initially
                ...
                // Set the index into the data gap list to the first gap. If only one log file is
                // loaded then this is unused
                int gapIndex = 0;
                ...
                for (PerfLog log : perfLog.getPerfLogData())
                {
                    ...
                    // Check if this log entry is the first following a data gap between two logs
                    if (gapIndex < perfLog.getDataGapIndex().size()
                        && log.equals(perfLog.getPerfLogData().get(perfLog.getDataGapIndex().get(gapIndex))))
                    {
                        // Determine the number of IDs that are active at the beginning of this gap
                    ...
                    if (pid.isPlotPerfID())
                    {
                        ...
                        if (activeIDCount == 0)
                        {
                            // Add a point to the CPU activity plot showing that the CPU is idle
                            cpuActivitySeries.add(log.getTimeStamp(), EXIT, false);
                        }
                        // Check if the active ID count went from 0 to 1, which implies that the
                        // CPU just became active
                        else if (activeIDCount == 1 && log.isEntryEvent())
                        {
                            // Add a point to the CPU activity plot showing that the CPU is active
                            cpuActivitySeries.add(log.getTimeStamp(), ENTRY, false);
                        }
                    }
                }
            }
            ...
            {
                // Plot the initial point showing the CPU is inactive. The end point is handled
                // below
                cpuActivitySeries.add(0.0, EXIT, false);
            }

            // Check that at least one data point has been added to the plot line
            if (cpuActivitySeries.getItemCount() > 0)
            {
                ...
                // Insert the CPU activity series as the first one shown in the combined plot
        ...
        // Set the amount to add to either side of the logged data minimum and maximum x-axis
        // values to provide some blank space between the edges of the plot and the first/last data
        // points
