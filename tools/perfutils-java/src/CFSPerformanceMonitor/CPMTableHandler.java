...
public abstract class CPMTableHandler extends JTable
{
    ...
    protected CPMTableHandler()
    {
        ...
        // Set the number of columns at which to wrap the a multi-line column to the default value
    ...
    protected class IconColumn
    {
        ...
        /******************************************************************************************
         * Return the icon to display in this cell, based on the column type and cell value
         *
         * @param value
         *            cell value
         *
    ...
    /**********************************************************************************************
     * Set the minimum number of columns at which to wrap the text in a multi-line column
     *********************************************************************************************/
     ...
     *
     * @return Minimum width, in pixels, required to display the longest value for a column
     *********************************************************************************************/
    ...
    protected JTableHeader createDefaultTableHeader()
    {
        return new JTableHeader(columnModel)
        {
            ...
            {
                // Get the tool tip for the column header underneath the mouse cursor
    ...
    /**********************************************************************************************
     * Set common table parameters and characteristics (for tables without special columns)
     *
     ...
     *
     * @param ignoreFocus
     *            true to ignore if the table has focus when determining the row colors
     *
     * @param multilineColumn
     *            column number to display multi-line text
     *
     * @param padAllColumns
     *            true if all columns should have a space prepended to the cell contents (doesn't
     *            affect index and color cells)
     *
     * @param tooltips
     *            tool tip text to display when hovering over the column headers
     *********************************************************************************************/
     ...
     *
     * @param idColumn
     *            ID column number. Set to COLUMN_NOT_DEFINED if none of the table's columns
     *            contain IDs
     *
     * @param nameColumn
     *            ID name column number. Set to COLUMN_NOT_DEFINED if none of the table's columns
     *            contain ID names
     *
     * @param notesColumn
     *            column number to display multi-line text, such as user-supplied annotations
     *
     * @param showColumn
     *            show ID in plots column number. Set to COLUMN_NOT_DEFINED if none of the table's
     *            columns contain Show IDs check boxes
     *
     * @param colorColumn
     *            ID color column number. Set to COLUMN_NOT_DEFINED if none of the table's columns
     *            contain color icons
     *
     * @param rowNumberColumn
     *            ID index column number. Set to COLUMN_NOT_DEFINED if none of the table's columns
     *            contain index values
     *
     * @param padAllColumns
     *            true if all columns should have a space prepended to the cell contents (doesn't
     *            affect index and color cells)
     *
     * @param iconColumns
     *            list of IconColumn objects containing information for the columns that should be
     *            displayed as icons instead of as text
     *
     * @param tooltips
     *            tool tip text to display when hovering over the column headers
     *********************************************************************************************/
                                           ...
                                           String[] tooltips)
    {
        ...
        // Set the table header font and calculate the header height. The column widths are
        ...
        // to be adjusted (e.g., loading a new log)
        setModel(new DefaultTableModel()
        {
            /**************************************************************************************
             * Override getColumnClass to allow sorting of the Statistics "Entry" and "Exit"
             * columns in numerical (versus string) order and to allow check boxes to be displayed
             * in the ID Editor "Show" column
             *************************************************************************************/
            @Override
     ...
     *
     * @param showScrollBar
     *            true if the table width must account for a vertical scroll bar
     *
                                              ...
                                              Boolean calcTotalWidth)
    {
        ...
        // Initialize the row sorter to null in order to prevent an error when the table is empty
        setRowSorter(null);
        ...
        if (calcTotalWidth)
        {
            // Calculate and set the widths of the tab columns and get the minimum width needed to
            // display the tab's table
    ...
    protected void setTableSortable()
    {
        ...
        // The table is empty; disable sorting to prevent an error that would occur if sorting was
        // attempted
        else
        {
            setRowSorter(null);
    ...
    {
        // Create a cell renderer to position the text in the specified columns and remove the
        // focus border
        ...
        // Set the logs table's column size; all but the Name and Notes columns have fixed widths
        for (int column = 0; column < getColumnModel().getColumnCount(); column++)
        {
            ...
            {
                // Use a cell renderer to remove the focus border and make the column resizable
            ...
            else
            {
                ...
                // Only apply the cell renderer to columns that are not displayed as check boxes
                if (column != showColumn)
                {
                    // Use the cell renderer to center the text in the column and remove the focus
                    // border
    ...
    /**********************************************************************************************
     * Calculate and then set the minimum width for each column and return the combined width of
     * all of the columns
     *
     * @param showScrollBar
     *            true if the table width must account for a vertical scroll bar
    ...
    private int calcColumnWidths(boolean showScrollBar)
    {
        ...
        for (int column = 0; column < getColumnModel().getColumnCount(); column++)
        {
            ...
            // Check that this column isn't displaying Color values. The colors column uses buttons
            // to show the color. The renderer, however, looks at the text representation of the
            // cell object which would cause the width to be sized too large. The Color column's
            // header text width is sufficient to size the column
            if (column != colorColumn)
            {
                // Step through each row in the table
                for (int row = 0; row < getRowCount(); row++)
                {
                    // Get the component used to draw the table cell from the cell's renderer. The
                    // Notes column uses a multi-line renderer which doesn't return the correct
                    // information; therefore a default renderer is used for this case
                    ...
                    // if it's larger. The width returned by getPreferredSize() includes the cell
                    // padding, so it doesn't have to be added here
                    minimumWidth[column] = Math.max(comp.getPreferredSize().width + 2,
                                                    minimumWidth[column]);
                }
            }

            // Check if this is the ID or ID name column. If no row exists for this table then the
            // column width is determined by a default value
            if (column == idColumn || column == nameColumn)
            {
                // Get the component used to draw the table cell from the cell's renderer and use
                // it to determine the width of the cell in pixels. Store this value if it's wider
                // than the largest width found so far
            ...
            {
                // Calculate the starting width of the Notes column. If the length of the note
                // exceeds a specified value then wrap the text at the value
        ...
        // Return the total width of the table. Include space for the table's vertical scroll bar,
        // if present
    ...
    /**********************************************************************************************
     * Set the keys that initiate editing the cell with the focus and which keys do not
     *********************************************************************************************/
    private void setEditorKeys()
    {
        // Add a key press listener to filter out invalid editor initiation keys
                ...
                if (!ke.isAltDown())
                {
                    switch (ke.getKeyCode())
                    {
                        ...
                        case KeyEvent.VK_C:
                            ...
                            if (ke.isControlDown())
                            {
                                // Allow Ctrl+A and Ctrl+C key sequences to remain active for
                                // selecting all table text and copying table text to the
    ...
    /**********************************************************************************************
     * Set the characteristics of a cell's text area for displaying word-wrapped text
     *********************************************************************************************/
    private void setMultilineEditorCharacteristics(JTextArea textArea)
    {
        ...
        // Add some space around the text's perimeter to separate it from the text area's border
    ...
    {
        // Create a text field so that its properties can be set and then used to create a default
        // editor for cells containing text
        ...
        for (int column = 0; column < getColumnCount(); column++)
        {
            ...
            if (isCellEditable(0, column))
            {
                ...
                {
                    // Set the editor so that a text entry pop-up dialog appears when a Notes
                ...
                {
                    // Set the editor so that a color choice dialog appears when a Color column
                    // cell is selected
                    getColumnModel().getColumn(column).setCellEditor(new ColorEditor());
                }
                // Check that this column doesn't contain the Show ID check boxes (e.g., Name, ID,
                // or Freq columns)
    ...
    /**********************************************************************************************
     * Class for displaying a text input dialog when a cell in the Notes column is selected
     *********************************************************************************************/
    private class TablePopupEditor extends DefaultCellEditor implements TableCellEditor
    {
        ...
        private TablePopupEditor()
        {
            ...
            // Create a text area to overlay the cell being edited. The overlay characteristics are
            // set to mimic the cell's contents so that the cell appears unchanged while the dialog
            // is open (otherwise the cell's text is reduced to a single line). The overlay also
        ...
        /******************************************************************************************
         * Display a text input dialog to edit the contents of a table's cell. Update the cell with
         * the text if the user selects the input dialog's Okay button
         *****************************************************************************************/
        @Override
        public Component getTableCellEditorComponent(final JTable table,
                                                     ...
                                                     int column)
        {
            // This is required so that the cell overlay component will be on the screen when its
            // location is used to determine the dialog's position
            SwingUtilities.invokeLater(new Runnable()
            {
                @Override
                public void run()
                {
                    // Set the number of rows in the text input area based on the length of the
                    // text. It's wrapped in a ScrollPane, so the text will "fit" regardless
                    int numRows = currentText.length() / minimumWrapColumn;

                    // Keep the number of rows within set bounds to prevent the text input dialog
                    ...
                    // Reset the text area focus traversal keys to use the default response to the
                    // Tab and Shift+Tab key presses (i.e., focus change) instead of inserting a
                    // tab into the text
                    ...
                    // Put the text area into a scroll pane to accommodate lengthy text. Then put
                    // the scroll pane into a panel so that a border can be applied. The layout
                    // allows the text area to resize to fit the dialog if it's resized
                    ...
                    if (new CPMDialogHandler().showOptionsDialog(cellOverlay,
                                                                 ...
                                                                 true) == OK_OPTION)
                    {
                        // User selected the Okay button; get the text that was entered
            ...
            // Set the overlay text so that it's the same as the cell's starting text
    ...
    /**********************************************************************************************
     * Class for displaying a color choice dialog when a cell in the Color column is selected
     *********************************************************************************************/
    private class ColorEditor extends AbstractCellEditor implements TableCellEditor
    {
        /******************************************************************************************
         * Display a color choice dialog and update the ID color if a valid color is selected
         *****************************************************************************************/
        @Override
        public Component getTableCellEditorComponent(final JTable table,
                                                     Object value,
                                                     ...
                                                     final int column)
        {
            // Set the selected row to the one containing the chosen Color cell and the focus to
            // the Color cell. This is in case a cell is chosen in a row other than the currently
            // selected row
        ...
        /******************************************************************************************
         * This is unused for the color editor, but this method must be provided since
    ...
    private DefaultTableCellRenderer setTableCellRenderer()
    {
        ...
        return new DefaultTableCellRenderer()
        {
            ...
            /**************************************************************************************
             * Override this method so that the row number is always displayed in the Row column, a
             * color box is displayed in the Color column, and to pad the name in the Name column
             *************************************************************************************/
            @Override
                                                           ...
                                                           int column)
            {
                ...
                // Set the icon to null. This prevents an icon from appearing in every cell of the
                // table
                setIcon(null);
                ...
                {
                    // Set the cell's text to the indicated row number (add 1 so that numbering
                    // begins at 1)
                ...
                else if (iconColumns != null)
                {
                    ...
                    {
                        // Check if the column matches one of the icon display columns
                        if (column == ic.column)
                        {
                            // Set the display icon based on the column type and value
    ...
    /**********************************************************************************************
     * Create a cell renderer in order to adjust the row height to fit the text and to remove the
     * focus border
     *********************************************************************************************/
    private class MultilineCellRenderer extends JTextArea implements TableCellRenderer
    {
        ...
        /******************************************************************************************
         * Override this method so that the row height can be adjusted to fit the text
         *****************************************************************************************/
        @Override
        public Component getTableCellRendererComponent(JTable table,
                                                       Object value,
                                                       ...
                                                       int column)
        {
            ...
            // Set the text area's width to the table column width. This is needed so that the
            // JTextArea calculates the height required to show all of the cell's text
                                     ...
                                     int column)
    {
        ...
        // Check if this is one of the selected rows and, if the focus isn't being ignored, that
        // the table has the focus
        ...
        else
        {
            ...
            if (column == showColumn)
            {
                // Set the cell opaque flag, which is based on the selected look & feel and if the
                // row number is odd (in order to alternate the row colors)
    ...
    private class TableCellListener implements PropertyChangeListener, Runnable
    {
        ...
        public void propertyChange(PropertyChangeEvent pce)
        {
            ...
            if (pce.getPropertyName().equals("tableCellEditor"))
            {
                ...
                // Editing has completed on this cell. Ignore changes if this cell is a Show ID
                // check box
                else if (editColumn != showColumn)
                {
                    // Get the cell value from the table; delete leading and trailing whitespace
        ...
        public void run()
        {
            ...
            if (!isEditing())
            {
                // Initiate editing on the selected cell. This cover a specific case where the
                // editor doesn't engage following resizing (smaller) a single table column
                editCellAt(editRow, editColumn);
            }

            // Convert the row index to the table model index, in case the rows have been sorted.
            // Since column swapping isn't allowed by the application there is no need to convert
            // the column index
            editRow = convertRowIndexToModel(editRow);

            // Store the cell's original contents so that he cell can be restored if the new value
            // is invalid or abandoned (by the user pressing the ESC key)
