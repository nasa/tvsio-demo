...
public class CPMLogHeaderDialog extends CPMDialogHandler
{
    ...
    private void initialize()
    {
        ...
        // Create a panel to hold the components of the dialog. Use a BorderLayout so that the
        // panel (and the tabbed pane within) fills the dialog if the dialog is resized
        ...
        for (LogHeader lh : perfLog.getLogHeader())
        {
            ...
            headerTable[index] = new CPMTableHandler()
            {
                ...
                protected void loadAndFormatData()
                {
                    ...
                    // Place the log header data into the table model, set up the renderers for the
                    // table cells, set up the table grid lines, and calculate the minimum width
                    // required to display the tab information
            ...
            // Set the number of columns to at which to begin wrapping the values column text based
            // on the log description length. If the description is very long the use a value that
            // is equal to the number of columns to display four 8-digit hexadecimal mask values (=
            // (8 chars + 1 space) * 4 mask values + 2 spaces for padding)
            headerTable[index].setMinimumWrapColumn(Math.min(lh.getDescription().length() + 2,
                                                             38));

            // Set the number of rows to display in the table to show all but the last two (filter
            // and trigger mask) rows
    ...
    /**********************************************************************************************
     * Print the log header table (in the currently selected tab if multiple logs are loaded)
     *
