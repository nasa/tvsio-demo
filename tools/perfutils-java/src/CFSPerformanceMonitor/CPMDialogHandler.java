...
public class CPMDialogHandler extends JDialog
{
    ...
    /**********************************************************************************************
     * Display a non-modal, resizable, user-interactive dialog with no title, using buttons defined
     * by the PRINT_OPTION option type. Return the button type selected
     *
     * @param parent
     *            window to center the dialog over
     *
    ...
    /**********************************************************************************************
     * Display a non-resizable, modal, user-interactive dialog with Append, Clear, and Cancel
     * buttons. Return the button type selected
     *
     * @param parent
     *            window to center the dialog over
     *
    ...
    /**********************************************************************************************
     * Display a non-resizable, modal, user-interactive color choice dialog using buttons defined
     * by the supplied option type and return the color selected (null if the Cancel button is
     * pressed)
     *
     * @param parent
     *            window to center the dialog over
                                    ...
                                    Color initialColor)
    {
        ...
        // Create a color chooser and set the selected color to the initial color
        ...
        // Display the color choice dialog and check if the user selected the OK button
     ...
     *
     * @param fileKey
     *            key to extract the file name(s) from the program preferences backing store
     *
     * @param optionType
     *            dialog type: LOAD_OPTION, SAVE_OPTION, SEARCH_OPTION, READ_OPTION, PRINT_OPTION,
     *            CLOSE_OPTION, OK_OPTION, or OK_CANCEL_OPTION
     *
     * @return Array containing the selected file handle(s) (null if the Cancel button is pressed
     *         or if the file name list is empty)
     *********************************************************************************************/
                                    ...
                                    int optionType)
    {
        ...
        // Check if no file/path name is provided. If not then the user is asked to choose one (or
        // more, if multiple selection is set)
        if (fileName == null || fileName[0] == null)
        {
            String nameList = "";

            // Create the file chooser. Assume the default path initially; if a path is provided
            // it's set below
            final JFileChooser chooser = new JFileChooser();

            // Read the file/path from the program preferences backing store (each file name is
            // initialized to "" in case it isn't found in the backing store; this prevents sending
            // a null string to the split method). Since this could be a multiple log file string,
            // use 'split' to break up the files and use the first file for the path. This has no
            // effect if only a single file/path is found, and returns "" if the file name entry
            // isn't found in the backing store
            ...
            // Check if the program preferences backing store has a specified file/path for the key
            // provided
            if (!filePath[0].isEmpty())
            {
                ...
                for (int index = 0; index < filePath.length; index++)
                {
                    ...
                    // Append the folder name if folder-only is allowed, else the file name without
                    // the path, to the name list, bounded by quotes
                    nameList += "\""
                                + (folderOnly
                ...
                // Use the file handles to initialize the list of files shown selected in the file
                // chooser. This does not highlight the files in all look & feels or operating
                // systems
                chooser.setSelectedFiles(tempFile);
            }

            // Locate the file name input field in the file chooser dialog. In order to use custom
            // buttons in the file chooser dialog, the file name input field must be located and
            // the inputs to it listened for so that the selected file can be updated. Otherwise,
            // changes made to the file name are ignored and when the Okay button is pressed the
            ...
            else
            {
                ...
                chooser.addPropertyChangeListener(new PropertyChangeListener()
                {
                     ...
                     * automatically for most look & feels, but not all (e.g., GTK+)
                     *****************************************************************************/
                    @Override
                    public void propertyChange(PropertyChangeEvent pce)
                    {
                        ...
                        if (pce.getPropertyName().equals("SelectedFilesChangedProperty"))
                        {
                            ...
                            {
                                // Append the file name without the path, surrounded by quotes, to
                                // the name list
                                nameList += "\""
            ...
            // Insert the file name list into the file chooser's file name text field. Most look &
            // feels do this automatically, but not all (e.g., GTK+)
            nameField.setText(nameList);
            ...
            if (createDialog(perfMain.getMainFrame(),
                             ...
                             true) == OK_OPTION)
            {
                // Extract the file name(s) from the file chooser name field. Spaces may appear in
                // the names only if the name is within double quotes. The names are split on
                // spaces outside of quotes, at every second quote, or at commas. First any leading
                ...
                // inside or outside a pair of quotes
                ...
                for (int index = 0; index < names.length(); index++)
                {
                    ...
                    // Check if the character is a space outside of a pair or quotes, or is the
                    // second quote of a pair of quotes
                    if ((c == ' ' || c == '"') && !isQuoted)
                    {
                        // Replace the character with a comma. Occurrences of double commas can
                        // result from this operation; these are accounted for later
                ...
                if (!fileName[0].isEmpty())
                {
                    ...
                    {
                        // Create a file handle for each file name or the path name. If this is not
                        // a folder, prepend the file path to the name
    ...
    /**********************************************************************************************
     * Fit the dialog containing a table to the contents and set the minimum size
     *
     * @param totalWidth
    ...
    {
        // If the table already exists then check if its minimum total width has grown larger. This
        // is always true for the initial instantiation
        ...
        // Save the table width. Use this to determine if the table needs to be resized after it is
        // already been made visible (i.e., if the minimum width gets larger due to longer ID names
        // or a look & feel change)
        tableWidth = totalWidth;
    }

    /**********************************************************************************************
     * Extract the dialog's button(s) from the supplied panel, then find the widest button
     * calculated from the button's text and icon
     *********************************************************************************************/
    protected void setButtonWidth()
    {
        ...
        for (int index = 0; index < numButtons; index++)
        {
            ...
            // Change the button padding between the button's icon/text and the perimeter of the
            // button
    ...
    /**********************************************************************************************
     * Create the buttons and button listeners for the dialog based on the option type provided
     *
     * @param optionType
     *            dialog type: LOAD_OPTION, SAVE_OPTION, SEARCH_OPTION, READ_OPTION, PRINT_OPTION,
     *            CLOSE_OPTION, OK_OPTION, or OK_CANCEL_OPTION
     *********************************************************************************************/
    private void createButtonPanel(int optionType)
    {
        // Create a button panel to hold the button(s)
        buttonPanel = new JPanel();

        // Determine the button text, number of buttons, and icon based on the type. Assume
        // initially there's an Okay and a Cancel button
        String okButtonText = "Okay";
        char okButtonMnemonic = 'O';
        String cancelButtonText = "Cancel";
        Icon buttonIcon = new ImageIcon(getClass().getResource(OK_ICON));
        int numButtons = 2;

        // Based on the dialog option type, adjust the button text and the number of buttons to
        // display
        switch (optionType)
        {
            case LOAD_OPTION:
            ...
            case PRINT_OPTION:
                ...
                buttonIcon = new ImageIcon(getClass().getResource(PRINT_ICON));
                break;

            case CLOSE_OPTION:
                okButtonText = "Close";
                okButtonMnemonic = 'C';
                buttonIcon = new ImageIcon(getClass().getResource(CLOSE_ICON));
                numButtons = 1;
                break;
        }
        ...
        btnOkButton.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent ae)
            {
                // Verify that the dialog box selection(s) are valid. This is always true unless
                // the verification method is overridden
                if (verifySelection())
                {
    ...
    private int createDialog(Component parent,
                             ...
                             boolean modal)
    {
        // Set the button panel to the supplied lower panel. buttonPanel is needed to be global in
        // order to resize the buttons for the case where the look & feel changes while the dialog
        // is open
        ...
        if (buttonPanel == null)
        {
            ...
            createButtonPanel(optionType);
        }

        // Size and position the dialog's button(s)
        setButtonWidth();

        // Add a change listener to the keyboard focus manager so component focus changes can be
        // detected
        KeyboardFocusManager.getCurrentKeyboardFocusManager().addPropertyChangeListener(new PropertyChangeListener()
        {
            ...
            /**************************************************************************************
             * Highlight a button if it has the keyboard focus and remove the highlight when it
             * loses focus
             *************************************************************************************/
            @Override
            public void propertyChange(PropertyChangeEvent pce)
        ...
        // remains centered as the dialog is resized
        ...
        // Set the layout manager for the dialog and and the upper and lower components
        ...
        // automatically exit the dialog. Instead, if this close button is pressed a button press
        // event is sent to the last button on the dialog's button panel
        setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);

        // Get the last button on the button panel. This is the button that closes/cancels the
        // dialog
        ...
        addWindowListener(new WindowAdapter()
        {
            ...
            {
                // Send a button press event for the dialog's close/cancel button. This allows the
                // frame's close button to perform the same actions as the dialog's close/cancel
                // button
                exitButton.doClick();
            }
        });
        ...
        // Check if the dialog is resizable and if no minimum size has been set for it
        if (resizable && !minimumSizeSet)
        {
            // Set the dialog minimum size to its current size. This prevents having a dialog that
            // can be resized too small to show its components
