...
public class CPMStatisticsPlot extends JFrame
{
    ...
    // Statistics plot currently selected by the user from the Plots | Statistics sub-menu or combo
    // box
    private static int selectedStat = 0;

    // Flag that is set to true to ignore the statistics plot type combo box action listener
    private boolean ignoreEvent;

    // Statistics plot names. This array is defined as static so that the Preferences dialog can
    // use the same values to create its plot selection combo box
    ...
    /**********************************************************************************************
     * This class is used to override the BarRenderer bar display color so that the bars are
     * displayed in the user-selected ID colors
     *********************************************************************************************/
    private class CPMBarRenderer extends BarRenderer
    {
        ...
        {
            // Get the ID color for the given bar. The scroll bar position determines the starting
            // index into the ID color array, and the column value provides the offset from the
            // uppermost bar (ID) visible
    ...
    private void createPlots()
    {
        ...
        // Select the statistics plot to display. This method also creates the pie and bar chart
        // data sets
        ...
        // Place the panel that contains the controls and chart into the main window frame
        perfMain.getStatsPlotPanel().add(controlAndChartPanel);

        // Set the size of the x-axis label plot pane. Validate causes the frame to determine the
        // sizes & positions of its components before they're displayed
    ...
    private void createPieChartProgressListener()
    {
        ...
        pieChartPanel.getChart().addProgressListener(new ChartProgressListener()
        {
            ...
            public void chartProgress(ChartProgressEvent cpe)
            {
                ...
                {
                    // Request focus for the plot window so that key presses can be detected. This
                    // call must be made after the plot window is displayed
    ...
    /**********************************************************************************************
     * Extend CategoryPlot to override its y-axis panning method so that the y-axis scroll bar
     * follows the plot's y-axis panning
     *********************************************************************************************/
    private class CPMCategoryPlot extends CategoryPlot
    {
        ...
        /******************************************************************************************
         * Pass the domain (y-axis) panning event through to its normal handler, then adjust the
         * y-axis scroll bar
         *****************************************************************************************/
        @Override
                                  ...
                                  Point2D source)
        {
            ...
            // Get a reference to the bar chart x-axis to shorten subsequent calls
            ...
            // Multiply the percentage change by an adjustment factor and add the result to the
            // running change total
            accumulator += percent * 50.0;

            // Convert the running change to an integer, which truncates the fractional portion
            ...
            // Determine the new y-axis scroll bar thumb position from the y-axis pan percentage
            // change
    ...
    {
        // Check that at least one ID is set to be displayed. If none are selected then the call
        // below results in an error if allowed to execute
        if (barDataset.getColumnCount() != 0)
        {
            // Set the first ID displayed based on the scroll bar's thumb position
    ...
    {
        // Create the domain axis (performance ID names) and set its sizing characteristics and
        // font
        ...
        // Create the range axis (statistics values) and set its tick label format, minor grid line
        // count, and font
        ...
        // Create a renderer to allow coloring the bars based on the user- selected ID colors.
        // Enable tool tips
        ...
        // Position the statistics value label at the bottom of the chart and make the minor grid
        // lines visible
        barPlot.setRangeAxisLocation(AxisLocation.BOTTOM_OR_LEFT);
        barPlot.setRangeMinorGridlinesVisible(true);

        // Set the range (x-axis; values) so it can be panned. The CategoryPlot class is extended
        // to allow panning of the domain (y-axis; names)
            ...
            public void chartProgress(ChartProgressEvent cpe)
            {
                ...
                if (cpe.getType() == ChartProgressEvent.DRAWING_FINISHED)
                {
                    ...
                    if (barChartHeight != controlAndChartPanel.getHeight())
                    {
                        ...
                        // Calculate the number of bars to display based on the height of the plot
                        // window, the label font height, plus some padding
                        int numBars = barChartHeight
                                      / (getFontMetrics(CELL_FONT).getHeight()
                                         + 10);

                        // Set the number of IDs visible based on main window height
                        ...
                        // Check if the scroll bar thumb extends past the bottom of the resized
                        // scroll bar
                        if (barScrollBar.getValue() + numBars > barDataset.getColumnCount())
                        {
                            // Adjust the scroll bar thumb position so that it remains within
                            // bounds
                            barScrollBar.setValue(barDataset.getColumnCount()
                                                  - numBars);
                        }

                        // Size and position the bar chart scroll bar based on the size of the
                        // chart. This has to be done after the chart is drawn since the values
                        // aren't known until this point
    ...
    private CPMChartPanel setChartCharacteristics(JFreeChart chart)
    {
        ...
        // Allow pie chart rotation and bar chart zooming and unzooming using the mouse wheel
    ...
    private void createPlotWindowComponents()
    {
        ...
        // Use an outer panel so that the components can be easily forced to the top of the
        // controls area
        ...
        // Create a sub-panel to hold the selection controls. This allows the sub-panel's
        // components to be positioned as desired relative to the other components in the controls
        // panel
        ...
        statsComboBox = new JComboBox<String>(PLOT_NAMES);
        ...
        statsComboBox.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent ae)
            {
                ...
                if (!ignoreEvent)
                {
                    // Since this could take a while, show the "wait" mouse pointer to alert the
                    // user. The pointer is restored to the default when the plot display completes
                    perfMain.setMainCursor(Cursor.WAIT_CURSOR);

                    // Save the index of the plot selected, then select that plot
        ...
        // Create radio buttons for displaying the statistics in pie or bar chart format
        ...
        ActionListener chartListener = new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent ae)
            {
                ...
                // Check if the chart type selected differs from the one currently in use
        ...
        // Create a checkbox for showing/hiding the CPU idle time. Initialize the box to the
        // Preferences dialog selection
        ...
        // Create radio buttons for sorting the statistics plot by ID/name or value
        ...
        // Adjust the border around the label so that the gap between the separator and the label
        // is the same as the gap in the Preferences dialog between the same two components. A
        // difference in the gap between the plot and the dialog can exist if there's a wider
        // border surrounding the radio button, which in the Preferences dialog is on the same row
        // as the label and therefore affects the spacing
        ...
        ActionListener sortListener = new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent ae)
            {
                // Get the "name" of the radio button selected (BY_NAME or BY_VALUE)
                int sortOrder = Integer.valueOf(((JRadioButton) ae.getSource()).getName());

                // Check if the sort order selected differs from the one currently in use
        ...
        // Controls and chart panel: chart selection combo box and other controls go on the left
        // side, selected chart goes on the right side
    ...
    private class CPMChartPanel extends ChartPanel
    {
        ...
        {
            // Set the bar chart range axis maximum derived from the selected statistic's maximum
            // value
        ...
        public void zoom(Rectangle2D zoomRectangle)
        {
            ...
            // Check if the difference between the upper boundary value is less than the minimum
            // allowed
    ...
    {
        // Calculate the new maximum x-axis boundary value based on the current value multiplied by
        // the zoom factor
        double maxX = barPlot.getRangeAxis().getRange().getUpperBound()
                      * zoomFactor;

        // Calculate the smallest allowed maximum chart boundary value based on the data's time
        // precision
        double minMaxX = MINIMUM_TIME_DELTA;

        // Check if the new maximum boundary value is less than the smallest value allowed
        if (maxX < minMaxX)
        {
            // Use the smallest allowable maximum value
            maxX = minMaxX;
        }
        // Check if the new maximum boundary value is greater than the maximum boundary value based
        // on the maximum statistical value displayed on this chart
    ...
    /**********************************************************************************************
     * Select a statistics plot to display and create the appropriate tool tip format
     *
     * @param plotIndex
    ...
    {
        // Save the index of the selected plot and update the combo box to reflect this selection.
        // Set the ignore flag so that the combo box action listener doesn't respond and call this
        // method again
        ...
        // Set up a tool tip to show the ID/ID name, pie slice value (with appropriate units), and
        // pie slice percentage whenever the mouse pointer hovers over a plotted point
    ...
    /**********************************************************************************************
     * Set the bar chart background and grid line colors based on the plot Preference dialog
     * selections
     *********************************************************************************************/
    ...
    {
        // Check if the user has selected to display the statistics data in a pie chart
        if (type == PIE)
        {
            // Set the radio button to 'selected'. This is necessary if the user toggles the "chart
            // type" radio button in the Preferences dialog; otherwise the local radio button will
            // be out of sync
            chartPieButton.setSelected(true);
            plotPrefs.setStatChartType(PIE);

            // Set the pie chart to display and remove the bar chart (if present)
        ...
        else
        {
            ...
            // Set the bar chart to display and remove the pie chart (if present)
    ...
    {
        // Adjust the check box selection. This is necessary if the user toggles the "Show idle"
        // check box in the Preferences dialog; otherwise the local check box will be out of sync
        cpuIdleCheckbox.setSelected(showIdle);
        plotPrefs.setShowCPUIdle(showIdle);

        // Check if the ID Time Active plot is selected. This is the only plot that can display the
        // CPU idle time
        if (selectedStat == TIME_ON)
        {
            // Update the pie chart based on if the CPU idle time is displayed. Check if the user
            // has selected to show the idle CPU time and that the idle time isn't already showing
            ...
            // Check if the user has selected to hide the idle CPU time and that the idle time
            // hasn't already been hidden
            ...
            // Rebuild the bar chart data set based on whether or not the CPU idle time is to be
            // included
            convertStatisticsToBarDataset();
    ...
    protected void setSortOrder(int sortOrder)
    {
        ...
        {
            // Set the radio button to 'selected'. This is necessary if the user toggles the "sort
            // by name" radio button in the Preferences dialog; otherwise the local radio button
            // will be out of sync
        ...
        {
            // Set the radio button to 'selected'. This is necessary if the user toggles the "sort
            // by value" radio button in the Preferences dialog; otherwise the local radio button
            // will be out of sync
    ...
    /**********************************************************************************************
     * Copy the statistics data to pie chart data set for displaying the statistics
     *********************************************************************************************/
    private void convertStatisticsToPieDataset()
    {
        ...
        // Changing the data set clears the currently selected sort order, so it must be restored
        // after rebuilding the data set. Check if the sort order is by ID name
        ...
        // Add the idle CPU time, if selected by the user and if appropriate for the plot selected
        setCPUIdleTime(plotPrefs.isShowCPUIdle(false));
    }

    /**********************************************************************************************
     * Copy the statistics data to bar chart data set based on the specified sort order
     *********************************************************************************************/
    private void convertStatisticsToBarDataset()
    {
        ...
        for (PerformanceID pid : perfIDs.getPerfIDList())
        {
            ...
            if (pid.isPlotPerfID())
            {
                // Based on the sort order selected, get the ID name or statistics value and add it
                // to the list
        ...
        // Check if the ID Time Active plot is selected and CPU idle time is set to be displayed.
        // This is the only plot that can display the CPU idle time
        if (selectedStat == TIME_ON && cpuIdleCheckbox.isSelected())
        {
            // Add the CPU idle time to the data set and its color to the color array. This is the
            // top item displayed, regardless of sort order
        ...
        // sorted list in order to prevent reusing an ID that has the same value as another ID
        ...
        for (int index = 0; index < sortedList.length; index++)
        {
            ...
            for (PerformanceID pid : perfIDs.getPerfIDList())
            {
                // Only add this ID's data if the user has it selected and if the ID hasn't already
                ...
                if (pid.isPlotPerfID() && !pid.isUsed())
                {
                    ...
                    // Set what to sort on (name or value) based on the current sort order
                    ...
                    {
                        // Set the flag indicating this ID has been used in the chart
                        pid.setUsed(true);
        ...
        // Determine the maximum statistical value and set the x-axis maximum boundary value
        maxXAxisValue = findMaximumStatisticsValue();
    }

    /**********************************************************************************************
     * Determine the maximum statistical value and use it to set the x-axis maximum boundary value
     *
    ...
    private Double findMaximumStatisticsValue()
    {
        ...
        for (int index = 0; index < barDataset.getColumnCount(); index++)
        {
            ...
            // Check if this is a valid value and if so, if it's the largest found so far
        ...
        // Check if no maximum was found. This would be due to having no IDs selected for display
        // or if the selected IDs do not have a value for the chosen statistic
        if (maxValue == 0.0)
        {
            // Set the maximum to a value greater than the minimum (0.0). This prevents zooming out
            // so far that an exception occurs due to the large value
            maxValue = 0.1;
        }
        // A maximum value was found
        else
        {
            // Pad the maximum value to provide some space at the right end of the plotted bar
            maxValue *= 1.05;
        }
     ...
     *            performance ID
     *
     * @return The value of the selected statistic for the performance ID specified
     *********************************************************************************************/
