...
public class CPMSetBoundsDialog extends CPMDialogHandler
{
    ...
    private JComboBox<String> minYComboBox;
    private JComboBox<String> maxYComboBox;
                                 ...
                                 CPMMain perfMain)
    {
        ...
        // Allow the user to select new x-axis boundaries. Perform a validity check before
                                 ...
                                 CPMMain perfMain)
    {
        ...
        // Allow the user to select new x- and y-axis boundaries. Perform a validity check before
        // implementing the updates
                                ...
                                tLabel.toArray(new String[tLabel.size()])) == OK_OPTION)
        {
            ...
            // Update the thread plot's y-axis boundaries. The label indices are biased slightly so
            // that the y-axis will include the minimum ID's exit point and the maximum ID's entry
            // point
     ...
     *
     * @param yAxisLabels
     *            string array containing the y-axis labels (null if the y-axis bounds are not
     *            displayed)
     *
                                    ...
                                    final String[] yAxisLabels)
    {
        ...
        if (yAxisLabels != null)
        {
            ...
            minYComboBox = new JComboBox<String>(yAxisLabels);
            ...
            maxYComboBox = new JComboBox<String>(yAxisLabels);
            ...
            {
                /**********************************************************************************
                 * Handle a minimum y combo box event. The list of maximum y IDs is adjusted so
                 * that IDs below the minimum do not appear in the maximum y combo box list
                 *********************************************************************************/
                @Override
                public void actionPerformed(ActionEvent ae)
                {
                    ...
                    if (!inhibitEvent)
                    {
                        // The setMaxYLabels() call causes multiple events to be sent to
                        // maxYComboBox. These events need not be acted on, so the inhibit flag is
                        // set to force these events to be ignored
                        inhibitEvent = true;
            ...
            {
                /**********************************************************************************
                 * Handle a maximum y combo box event. The list of minimum y IDs is adjusted so
                 * that IDs above the maximum do not appear in the minimum y combo box list
                 *********************************************************************************/
                @Override
                public void actionPerformed(ActionEvent ae)
                {
                    ...
                    if (!inhibitEvent)
                    {
                        // The setMinYLabels() call causes multiple events to be sent to
                        // minYComboBox. These events need not be acted on, so the inhibit flag is
                        // set to force these events to be ignored
                        inhibitEvent = true;
        ...
        // Since this dialog is modal it will wait here until the user selects Okay or Cancel.
     ...
     *
     * @param labels
     *            array of y-axis ID names (or ID if no name is supplied) up to an including the
     *            current maximum y-axis ID name
     *********************************************************************************************/
    private void setMinYLabels(String[] labels)
    {
        // Save the current minimum y selection so it can be restored after recreating the list
        ...
        // Rebuild the list based on the currently selected maximum y label. The valid minimum y
        // labels are those starting with the minimum possible y label and ending with the maximum
        // y label
        ...
        // Restore the minimum y selection to its original value; otherwise it would be reset to 0
     ...
     *
     * @param labels
     *            array of y-axis ID names (or ID if no name is supplied) beginning with the
     *            current minimum y-axis ID name
     *********************************************************************************************/
    private void setMaxYLabels(String[] labels)
    {
        // Save the current maximum y selection so it can be restored after recreating the list
        ...
        // Rebuild the list based on the currently selected minimum y label. The valid maximum y
        // labels are those starting with the minimum y and ending with the maximum possible y
        // label
        ...
        // Restore the maximum y selection to its originally selected label. Since the number of
        // items in the list changed, the original index isn't the same as the new index, hence the
        // calculation to determine the equivalent index in the new list
    ...
    protected boolean verifySelection()
    {
        ...
        // Ensure that the x-axis minimum and maximum fields contain numbers (not blank and no
        ...
        // Ensure that the x-axis minimum is less than the maximum. Y-axis labels don't require
        // checking since the combo box' contents limits the selection to legal values
        else if (Double.valueOf(newMinX) >= Double.valueOf(newMaxX))
        {
            // Inform the user that the minimum x-value must be less than the maximum x-value
            new CPMDialogHandler().showMessageDialog(this,
