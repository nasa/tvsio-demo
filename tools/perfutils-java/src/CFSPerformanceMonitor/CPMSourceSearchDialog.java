...
public class CPMSourceSearchDialog extends CPMDialogHandler
{
    ...
    private void initialize()
    {
        ...
        // Create a list of check boxes showing the target folder's subfolders. Get the list of
        // files and folders in the target folder and sort them alphabetically
        ...
        for (File item : folderItems)
        {
            ...
            if (item.isDirectory())
            {
                // Create a check box with the subfolder name as the label and add the check box to
                // the panel
        ...
        {
            // Indicate that the files within the target folder will be checked for IDs
        ...
        {
            // Set the number of rows in which to group the subfolder check boxes. This puts them
            // into a more or less square grid
            ((GridLayout) subPanel.getLayout()).setColumns((int) Math.sqrt(subfolderCheckBox.size()));
            ((GridLayout) subPanel.getLayout()).setRows(0);
        }

        // Set the selection statuses of the check boxes based on the preferences information
        ...
        btnOkay.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent ae)
            {
                ...
                if (verifySelection())
                {
                    // Build the list of subfolders to search from the selected check boxes
                    String subfolderNames = "";

                    // Check if no subfolders exist in the target folder
                    if (subfolderCheckBox.size() == 0)
                    {
                        // Set the target folder an the subfolder to check; i.e., search through
                        // the files within the target folder
                    ...
                    // Store the search criteria in the program preferences backing store
        ...
        btnDefault.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent ae)
            {
                // Update the check boxes and text fields to show the default values
                setSubfolderSelected();
        ...
        // Display the search criteria dialog and wait for the user to select the Okay or Cancel
        // button
    ...
    /**********************************************************************************************
     * Set the state of the subfolder check boxes based on the stored preference information
     *********************************************************************************************/
    private void setSubfolderSelected()
    {
        // Get the subfolder preference information and split it into separate subfolder names
    ...
    protected boolean verifySelection()
    {
        ...
        // Get the criteria from the input fields, and remove leading and trailing white space
        ...
        // Check if the target folder contains one or more subfolders and none is selected
        ...
        // Check if the file extension or ID suffix fields have spaces or commas
        else if (extension.contains(" ")
                 || extension.contains(",")
                 || suffix.contains(" ")
                 || suffix.contains(","))
        {
            // Let the user know that the extension and suffix fields can't contain spaces or
            // commas
            new CPMDialogHandler().showMessageDialog(this,
