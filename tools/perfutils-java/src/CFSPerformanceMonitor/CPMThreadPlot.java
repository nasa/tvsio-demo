...
public class CPMThreadPlot extends CPMXYPlotHandler
{
    ...
    /**********************************************************************************************
     * Extend XYPlot to override its panning methods so that the x- and y-axes scroll bars follow
     * the plot's x- and y-axes panning
     *********************************************************************************************/
    ...
    private class CPMXYPlot extends XYPlot
    {
         ...
         * x-axis label plot and scroll bar
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * Pass the range (y-axis) panning event through to its normal handler, then adjust the
         * y-axis scroll bar
         *****************************************************************************************/
        @Override
                                 ...
                                 Point2D source)
        {
            ...
            // Adjust the y-axis scroll bar thumb size, position, and increments
            adjustScrollBarIncrements();
        }
    }
    ...
    protected JFreeChart createPlotChart()
    {
        ...
        // Create the plot. Even though there is a single thread plot, List is needed so that the
        // CPMXYPlotHandler methods, which require a List object, can be used
        ...
        // Set the data set rendering order so that the ID plot points are drawn over the thread
        // line
        ...
        // Draw the horizontal and vertical grid lines based on the user's selection
    ...
    protected void convertLogDataToXYSeries()
    {
        ...
        // Make a pass through the IDs to save the ID colors, set the label names, and create the
        // plot series in the order specified by the ID list
        ...
        {
            // Set the plot's y-axis tick labels (first converting the List to an array). There's
            // only a single symbol axis for the thread plot, but a List is used so that the
            // CPMXYPlotHandler methods can be used
            getSymbolAxis().add(new SymbolAxis(null,
                                               yAxisLabel.toArray(new String[yAxisLabel.size()])));

            // Collect data from the log to populate the plot series for each individual ID (in
            // order to plot the end points) and for the thread of execution line
            for (PerfLog log : perfLog.getPerfLogData())
            {
                ...
                for (PerformanceID pid : perfIDs.getPerfIDList())
                {
                    ...
                    if (pid.isPlotPerfID())
                    {
                        ...
                        {
                            // Base the y-value on the ID index (to match up with the y-axis tick
                            // labels). Bias up or down based on if this is an entry or exit event
                            double yValue = log.isEntryEvent()
                                                               ? idIndex + 0.25
                                                               : idIndex - 0.25;

                            // Add the data point to this ID's plot line and the thread plot line
                            ...
                            // Since the ID matched, quit looking through the ID list and go to the
                            // next log entry
                            break;
                        }
        ...
        {
            // Create a dummy symbol axis so that the plot won't generate an exception when
            // displayed
