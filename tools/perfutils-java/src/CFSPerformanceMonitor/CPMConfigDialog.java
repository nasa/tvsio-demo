...
public class CPMConfigDialog extends CPMDialogHandler
{
    ...
    // True if the ID and log file names from the config selected in the combo box should be
    // displayed; false to display the currently loaded ID and log file names
     ...
     *
     * @param dialogType
     *            config dialog type: ConfigType.LOAD, ConfigType.Save, or ConfigType.Delete
     *********************************************************************************************/
    protected CPMConfigDialog(CPMMain perfMain, ConfigType dialogType)
    {
    ...
    private void initialize()
    {
        ...
        // Create the config dialog used by the Load Config, Save Config, and Delete Config menu
        // items
        configPanel = new JPanel(new GridBagLayout());
        configSelLabel = new JLabel();
        configSelLabel.setFont(LABEL_FONT_BOLD);
        configComboBox = new JComboBox<String>();
        ...
        // Get the default background color; this is used to color the ID and log file text areas
        ...
        // Get the list of configs and add these to the combo box, and if a config has been
        // previously chosen then initially select it
        configComboBox.setModel(new DefaultComboBoxModel<String>(getConfigList()));
        configComboBox.setSelectedItem(perfMain.getSelectedConfig());

        // Create the string containing the ID and log file names and display it underneath the
        // combo box
        ...
        configComboBox.addActionListener(new ActionListener()
        {
            ...
            {
                // Create the string containing the ID and log file names and display it underneath
                // the combo box
                configFilesTextArea.setText(parseConfigFileNames());

                // Resize the dialog, if needed, to fit the number and length of the config file
                // information
        ...
        // Keep showing the dialog as long as the saved config name is invalid (if this is a Save
        // Config dialog) or the user selects Cancel
        while (errorFlag)
        {
            // Assume the saved config name is valid (always true for the Load and Delete Configs
            // dialogs)
            errorFlag = false;

            // Display the Configs dialog and wait for the user to select the Okay or Cancel button
            ...
            {
                // Get the config name and remove any white space surrounding it
    ...
    protected boolean verifySelection()
    {
        ...
        if (dialogType == SAVE)
        {
            // Check if the config name, after removing any whitespace surrounding it, is blank
    ...
    /**********************************************************************************************
     * Read the sorted list of configurations from the program preferences backing store
     *
     * @return Array of configs, or a single item array containing an empty string if no configs
     *         are found
     *********************************************************************************************/
    private String[] getConfigList()
    {
        ...
        {
            // Inform the user that an error occurred accessing the preference keys
            new CPMDialogHandler().showMessageDialog(perfMain.getMainFrame(),
        ...
        {
            // Create an array with an empty string so that the caller has something to work with
    ...
    /**********************************************************************************************
     * Parse the currently selected config string from the program preferences backing store to
     * extract the ID and log file names and return the names to the caller, formatted for display
     * in a column
     *
     * @return String showing the ID file name and the log file name(s), separated by carriage
     *         returns
     *********************************************************************************************/
    private String parseConfigFileNames()
    {
        ...
        // There may be multiple log file names, so divide the log file portion of the string into
        // parts using the specified delimiter
        ...
        {
            // Remove the path name and store the log file name in the return string
            files += "\n"
