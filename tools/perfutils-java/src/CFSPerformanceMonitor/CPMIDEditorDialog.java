...
public class CPMIDEditorDialog extends CPMDialogHandler
{
    ...
    private void initialize()
    {
        ...
        {
            /**************************************************************************************
             * Override isCellEditable so that only the Show, ID, Name, Color, Frequency, and Notes
                                                   ...
                                                   Object newValue)
            {
                ...
                if (column == ID)
                {
                    ...
                    // Check if the ID can be converted from a string to an integer
                    ...
                    {
                        // Check if the ID exceeds the maximum value supported by the current log
                        if (perfLog.getMaximumIDs() != 0
                            && perfID >= perfLog.getMaximumIDs())
                        {
                            // Inform the user that the ID is larger than the maximum supported
                            new CPMDialogHandler().showMessageDialog(CPMIDEditorDialog.this,
                                                                     "<html><b>ID \""
                                                                                             + newValueS
                        ...
                        // Compare this ID to the others in the table in order to avoid creating a
                        // duplicate
                        for (int otherRow = 0; otherRow < getRowCount(); otherRow++)
                        {
                            // Only compare if this row isn't the one being edited
                            if (otherRow != viewRow)
                            {
                                if (perfIDs.convertIDToHexString(perfID).equals(String.valueOf(getValueAt(otherRow, ID))))
                                {
                                    // Inform the user that this ID matches one in another row
                                    new CPMDialogHandler().showMessageDialog(CPMIDEditorDialog.this,
                                                                             "<html><b>Duplicate ID \""
                                                                                                     + newValueS
                                                                                                     + "\" - matches row "
                    ...
                    if (!errorFlag)
                    {
                        ...
                        // Set the filter and trigger mask columns based on the log file masks
                ...
                {
                    // Check if the ID name isn't blank. It's okay to have multiple IDs with no
                    // name, so only perform the duplicate name test if the user has entered a name
                    if (!newValue.equals(""))
                    {
                        // Compare this ID name to the others in the table in order to avoid
                        // creating a duplicate
                        for (int otherRow = 0; otherRow < getRowCount(); otherRow++)
                        {
                            // Only compare if this row isn't the one being edited, and if the ID
                            // name isn't the default name
                            if (otherRow != viewRow
                                && newValue.equals(getValueAt(otherRow, NAME)))
                            {
                                // Inform the user that this ID name matches one in another row
                                new CPMDialogHandler().showMessageDialog(CPMIDEditorDialog.this,
                ...
                else if (column == FREQ)
                {
                    ...
                    // Check if the frequency contains other than a positive decimal number. Values
                    // can be in the format #.#, #., .#, -#, -#., -.#, or -#.#
                    if (!newValueS.matches("(\\d*\\.?\\d+)|(\\d+\\.?\\d*)"))
                    {
                        // Inform the user that the frequency is in the wrong format
                        new CPMDialogHandler().showMessageDialog(CPMIDEditorDialog.this,
                    ...
                    if (!errorFlag)
                    {
                        // Parse the frequency and format it, removing any excess leading or
                        // trailing zeroes
            ...
            {
                // Copy the performance ID list contents to an object array for display and
                // manipulation in the dialog box table
                ...
                for (PerformanceID pid : perfIDs.getPerfIDList())
                {
                    ...
                    // This flag indicates if the ID is to be plotted or not (status is shown in a
                    // user-selectable checkbox)
                    tablePIDList[row][SHOW] = pid.isPlotPerfID();

                    // Compare the ID to the log's filter and trigger masks; if the ID's mask bit
                    // is set then indicate it's in the mask
                    ...
                    // Convert the ID to an 8-character hexadecimal string with '0x' prepended
                    tablePIDList[row][ID] = perfIDs.convertIDToHexString(pid.getPerfID());

                    // ID name
                    tablePIDList[row][NAME] = pid.getPerfIDName();

                    // ID schedule frequency. Format the value, removing any excess trailing zeroes
                    tablePIDList[row][FREQ] = formatFrequency(pid.getPerfIDFreq());

                    // Number of logged entry and exit events. This is included so that the IDs can
                    // be sorted by this attribute
                ...
                // Place the ID data into the table model along with the column names, set up the
        ...
        // Create the list containing the columns that display icons and the icons to display,
        ...
        // Create an outer panel to put the editor panel in. The border doesn't appear without this
            ...
            public void actionPerformed(ActionEvent ae)
            {
                ...
                if (tableHasChanged())
                {
                    ...
                    if (new CPMDialogHandler().showMessageDialog(CPMIDEditorDialog.this,
                                                                 ...
                                                                 OK_CANCEL_OPTION) == OK_OPTION)
                    {
                        // Restore the original ID data into the table model, reset the editors and
                        // renderers for the table cells, and reset the table grid lines
        ...
        btnSave.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent ae)
            {
                // Only save the IDs if a cell's content has changed and that every row with an ID
                // name entered also has an ID
                if (tableHasChanged() && !checkForMissingID())
                {
                    ...
                    for (int row = 0; row < idTable.getRowCount(); row++)
                    {
                        ...
                        {
                            // Copy the table contents into the performance ID list
                    ...
                    // Set the "changes saved" flag so that remapping the ID list will be performed
        ...
        btnClose.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent ae)
            {
                // Check if the table contents hasn't changed, or if it has (due to unsaved
                // changes) then confirm discarding the changes
                if (!tableHasChanged()
                    || new CPMDialogHandler().showMessageDialog(CPMIDEditorDialog.this,
                                                                ...
                                                                OK_CANCEL_OPTION) == OK_OPTION)
                {
                    ...
                    if (changesSaved)
                    {
                        // Remap the performance IDs to the log. This can't be done during the save
                        // operation since, if the user changes an ID that's in the log, the ID
                        // from the log is reloaded, which changes the list and causes a miscompare
                        // on close
    ...
    private void removeRows(int startRow, int endRow)
    {
        ...
        if (model.getRowCount() != 0)
        {
            // Counter for the number of rows skipped due to the IDs having logged events
            int numRowsSkipped = 0;

            // Step through each row in the table, starting at the end. Going in reverse simplifies
            // maintaining the correct index if rows are retained due to having events
            while (endRow >= startRow)
            {
                // Retrieve the contents of the Events, ID, and mask columns for the current row
                ...
                {
                    // Events exist for this ID. These would be automatically restored when the log
                    // is remapped to the IDs, so insert the ID back into the table with the
                    // user-adjustable parameters reset to the defaults
                    ...
                    // Increment the the skip counter. This is used to determine if a warning
            ...
            // Check if any rows remain in the table. If so, select the row at the starting index.
            // Also select any rows that were not deleted (due to having logged events)
            if (idTable.getRowCount() != 0)
            {
                ...
                // Determine the ending selected row based on the number of skipped rows
                endRow = startRow
                         + (numRowsSkipped == 0
                                                ? 0
                                                : numRowsSkipped - 1);
    ...
    /**********************************************************************************************
     * Change the table model based to match the arrangement of the data in the table view
     *********************************************************************************************/
    private void updateTableModelToView()
    {
        ...
        // Replace the original ID data in the table model with the new (sorted) model data, and
    ...
    private boolean tableHasChanged()
    {
        ...
        for (PerformanceID pid : perfIDs.getPerfIDList())
        {
            ...
            // Check if the show flag, ID, name, color, frequency, or notes has been changed, or if
            // there are more entries in the list than in the table
            if (tableRow == idTable.getRowCount()
                || !idTable.getValueAt(tableRow, SHOW).toString().equals(Boolean.toString(pid.isPlotPerfID()))
                || !idTable.getValueAt(tableRow, ID).equals(perfIDs.convertIDToHexString(pid.getPerfID()))
                || !idTable.getValueAt(tableRow, NAME).toString().equals(pid.getPerfIDName())
                || !idTable.getValueAt(tableRow, COLOR).equals(pid.getPerfIDColor())
                || !idTable.getValueAt(tableRow, FREQ).toString().equals(formatFrequency(pid.getPerfIDFreq()))
                || !idTable.getValueAt(tableRow, NOTES).equals(String.valueOf(pid.getPerfIDNotes())))
            {
        ...
        // Check if there are any additional non-empty rows at the end of the table
        if (!hasChanges && tableRow < idTable.getRowCount())
        {
            ...
            if (tableRow != idTable.getRowCount())
            {
                // Since the end of the table was not reached, additional non-empty rows exist; set
                // the change flag
                hasChanges = true;
            }
        }
    ...
    /**********************************************************************************************
     * Determine if any changes will result to the ID colors if the colors are auto-assigned
     *********************************************************************************************/
    private boolean colorsWillChange()
    {
        ...
        for (int row = 0; row < idTable.getRowCount(); row++)
        {
            ...
            {
                // Set the flag to indicate the color will change and exit the loop
                colorChange = true;
                break;
            }
        }
    ...
    /**********************************************************************************************
     * Adjust the table row index to skip rows with no ID and no name. The default ID name is
     * equivalent to having no name
     *********************************************************************************************/
    private int skipEmptyTableRows(int tableRow)
    {
        // Check if the ID and name fields are empty, and that the end of the table hasn't been
        // reached
    ...
    /**********************************************************************************************
     * Format the floating point ID frequency value as a string, removing any excess trailing
     * zeroes
     *
     * @param freq
     *            ID frequency
