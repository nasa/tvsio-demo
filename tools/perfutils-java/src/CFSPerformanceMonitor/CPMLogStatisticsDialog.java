...
public class CPMLogStatisticsDialog extends CPMDialogHandler
{
    ...
    // Define globally so that the components can be accessed by the action listeners, etc.
    private CPMTableHandler statsTable;
    private CPMTableHandler totalsTable;

    // These components must be defined globally so they can be accessed by the dialog window
    // resize listener
    ...
    private void initialize()
    {
        ...
        statsTable = new CPMTableHandler()
        {
            ...
            /**************************************************************************************
             * Override getToolTipText so that the time stamp for the minimum and maximum statistic
            ...
            public String getToolTipText(MouseEvent me)
            {
                ...
                for (PerformanceID pid : perfIDs.getPerfIDList())
                {
                    if (pid.getPerfID() == perfIDs.convertStringToIntID(getValueAt(rowIndex,
                                                                                   ID).toString()))
                    {
                        // Create the tool tip text based on the table column over which the mouse
                        // pointer is located
                        switch (colIndex)
                        {
                            case MIN_ACT:
            ...
            /**********************************************************************************************
             * Statistics table loader placeholder. The statistics and totals tables are loaded and
             * formatted together in the totals table override for this method
             *********************************************************************************************/
            @Override
        ...
        {
            /**************************************************************************************
             * Override getToolTipText so that the time stamp for the overall minimum and maximum
             * statistic values can be displayed
             *************************************************************************************/
            @Override
            public String getToolTipText(MouseEvent me)
            {
            ...
            /**************************************************************************************
             * Load the ID statistics into the table and format the table cells. Both the
             * statistics and totals tables are loaded in this one method for simplicity
             *************************************************************************************/
            @Override
            protected void loadAndFormatData()
            {
                // Copy the performance ID statistics values to an object array for display in the
                // dialog box statistics table
                ...
                // Copy the overall statistics values to an object array for display in the dialog
                // box totals table
                ...
                // Store the row number (equals the total number of rows in the table)
                int maxRow = row;

                // Create an array to hold the maximum width, in characters, of each column
                int maxLen[] = new int[NUM_COLUMNS];

                // Pad the values in the cells with leading spaces so that the decimals align. Step
                // through the data columns
                for (int column = 0; column < NUM_COLUMNS; column++)
                {
                    // Check if this is a column containing floating point values
                    if (column >= AVG_FREQ)
                    {
                        // Step through each row in this column
                        for (row = 0; row < maxRow; row++)
                        {
                            // Check if the width of the text is the longest found
                        ...
                        // Check if the totals text width for this column is longer
                        ...
                        for (row = 0; row < maxRow; row++)
                        {
                            ...
                            {
                                // Pad the text with spaces (to the left) to make it the maximum
                                // width. This aligns the values by their decimal points
                        ...
                        {
                            // Pad the text with spaces (to the left) to make it the maximum width.
                            // This aligns the value by its decimal point with the values in the
                            // statistics table
                ...
                // Place the statistics data into the table model along with the column names, set
                // up the renderers for the table cells, set up the table grid lines, and calculate
                // the minimum width required to display the statistics information
                ...
                // Place the statistics totals data into the table model. The column names aren't
                // used but an array of the correct size must be supplied. Set up the renderers for
                // the table cells and the table grid lines
                ...
                // Fit the dialog to the table and set the minimum size based on the column widths
                setDialogMinimumSize(totalWidth);

                // Set the dialog's title so that it includes the log file name(s)
        ...
        // For each of the columns in the table that can be a floating point value
        for (int index = AVG_FREQ; index < NUM_COLUMNS; index++)
        {
            // Override the sort comparator to convert the strings to numbers, if applicable, and
            // then compare them. Otherwise these table entries would be sorted alphabetically
            sorter.setComparator(index, new Comparator<String>()
            {
                ...
                public int compare(String strg1, String strg2)
                {
                    ...
                    // Check if either string is "n/a"; if so, treat it as the greater value when
                    // sorting
                    ...
                    {
                        // Parse the strings as numbers and compare; sort based on the values
        ...
        // Create the totals table panel with fixed dimensions. The width value is unused
        ...
        {
            /**************************************************************************************
             * Handle the dialog window resize event so that the totals table can be positioned if
             * the window is resized using the mouse
             *************************************************************************************/
            @Override
    ...
    protected boolean verifySelection()
    {
        ...
        // Calculate the number of spaces with which to pad the "Overall" text in the table in
        // order to right-justify it in the Name column. This is equal to the column's pixel width
        // minus the cell padding and the width of the "Overall" text, divided by the width of a
        // single character (the cells use a fixed width font)
        ...
        // Since the data consists of two tables (statistics and totals), to print these together
        // requires combining the tables temporarily. Insert an empty row, then insert the totals
        // table contents into the statistics table
    ...
    /**********************************************************************************************
     * Reposition the totals table when the dialog box changes size, including when changes are
     * made to the look & feel
     *********************************************************************************************/
    private void repositionTotalsTable()
    {
        // Calculate the padding, in pixels, required to align the columns in the totals table with
        // those in the statistics table. The position is dependent on the current size of the ID
        // name column (which is resizable)
    ...
    /**********************************************************************************************
     * Format a statistic value. If the value hasn't been calculated then return "n/a"
     *
     * @param format
    ...
    /**********************************************************************************************
     * External method to reload the statistics into the tables and reformat the table cells
     *********************************************************************************************/
