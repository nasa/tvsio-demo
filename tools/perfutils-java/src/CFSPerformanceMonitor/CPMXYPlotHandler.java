...
public abstract class CPMXYPlotHandler
{
    ...
    // Amount to add to either side of the logged data minimum and maximum x-axis values; based on
    // a percentage of the total x-axis span
    ...
    // Cursor information mode start marker, end marker, and current mouse pointer x-axis
    // coordinates
    ...
    // Set to true if the plot has a y-axis scroll bar controlled by the custom methods contained
    // in this class; set to false otherwise
    private boolean yAxisCustomScrollBar;

    // Flag to indicate whether or not the chart progress listener should process events
    ...
    /**********************************************************************************************
     * Return the amount to add to either side of the logged data minimum and maximum x-axis
     * values; based on a percentage of the total x-axis span
     *
    ...
    /**********************************************************************************************
     * Placeholder for adjust the plot height and spacing (applicable to step plots)
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Create the plot components and listeners, convert the data into plot series format, and
     * display the plot
     *
     * @param plotPanel
     *            plot panel
    ...
    {
        // Since this could take a while, show the "wait" mouse pointer to alert the user. The
        // pointer is restored to the default when the plot display completes
        ...
        // Add a mouse button, motion, and wheel listener for the plot chart panel
        ...
        // Create and arrange the plot window components and place them into the plot panel
        plotPanel.add(createPlotWindowComponents());

        // Validate causes the frame to determine the sizes & positions of its components before
        // they're displayed
        plotPanel.validate();

        // Set the x-axis label plot pane height now that the sizes and positions of the other
    ...
    private void createCrosshairCursor()
    {
        ...
        if (cursorURL != null)
        {
            // Create the custom crosshair mouse pointer for use when over the plot window
    ...
    private void createCursorInfoFields()
    {
        ...
        // Create the cursor information display fields: start time, end time, and delta time. Base
        // the text field sizes on the font
        ...
        // Add a dummy text area so that space is created for the cursor information fields. The
        // actual fields are created once the width of the chart y-axis labels is known (i.e.,
    ...
    {
        // Create an empty border to surround the panes. This removes graphic artifacts that
        // otherwise can appear at the corners of the panes
        Border emptyBorder = BorderFactory.createEmptyBorder();

        // Create a border that adds padding between the plot's y-axis scroll bar and the edge of
        // the main window, and between the bottom of the plot and the x-axis label plot
        Border plotAndLabelBorder = BorderFactory.createEmptyBorder(0,
                                                                    0,
                                                                    PLOT_XAXIS_LABEL_PAD,
                                                                    PLOT_SCROLL_BAR_PAD);

        // Plot and label plot panel: The plot goes at the top and the x-axis label plot goes at
        // the bottom
        ...
        if (yAxisCustomScrollBar)
        {
            ...
            // Plot & y-axis scroll bar panel: Plots go on left side, plot y-axis scroll bar goes
            // on right side
            JPanel plotAndYBarPanel = new JPanel(new BorderLayout());
            plotAndYBarPanel.setBorder(plotAndLabelBorder);

            // Place the plot panel on the left side and the y-axis scroll bar on the right side of
            // the plot & y-axis scroll bar panel
            plotAndYBarPanel.add(plotChartPanel, BorderLayout.CENTER);
            plotAndYBarPanel.add(yAxisScrollBar, BorderLayout.LINE_END);

            // Place the plot & y-axis scroll bar panel at the top of the plot & label panel
        ...
        else
        {
            ...
            // Set y-axis scroll bar to the scroll pane's scroll bar. Set the scroll bar's unit
            // increment (otherwise the mouse and keyboard scroll different amounts)
        ...
        // Cursor & plot panel: Cursor information goes at the top, plot & x-axis scroll bar panel
        // goes at the bottom
        ...
        // Plot & x-axis scroll bar panel: Plots go at the top, plot x-axis scroll bar goes at the
        // bottom
        JPanel plotAndXBarPanel = new JPanel(new BorderLayout());
        plotAndXBarPanel.setBorder(emptyBorder);

        // Place the x-axis label chart at the bottom of the plot & label panel. The chart is first
        // placed into a panel so that the vertical size of the x-axis chart can be adjusted
        ...
        // Place the cursor information label at the top and the plot & x-axis panel in the lower
        // portion of the cursor & plot panel
        cursorAndPlotPanel.add(cursorPanel, BorderLayout.PAGE_START);
        cursorAndPlotPanel.add(plotAndXBarPanel, BorderLayout.CENTER);

        // X-axis scroll bar panel: This corrects a redraw issue when using the GTK+ look & feel
        // where the left side of the bar doesn't always meet the left arrow of the scroll bar
        ...
        // Place the plot & label panel at the top and the x-axis scroll bar (in its panel) at the
        // bottom of the plot & x-axis scroll bar panel
    ...
    {
        // Set up a tool tip to show the x-axis coordinate whenever the mouse cursor hovers over a
        // plotted point
    ...
    private void createChartPanels()
    {
        ...
        // Prevent zooming the y-axis unless the custom y-axis scroll bar is in use (the x-axis can
        // still be zoomed)
        ...
        // Set the zoom factors for the axes. By using inverse values for the zoom factors the
        // original axis range can be reestablished by zooming in the opposite direction (if +x%
        // and -x% were used then zooming one way and then the opposite way doesn't return to the
        // original values)
        plotChartPanel.setZoomInFactor(0.95);
        plotChartPanel.setZoomOutFactor(1.0 / 0.95);

        // Set the delay (in milliseconds) for showing the x-axis coordinate tool tip
        plotChartPanel.setInitialDelay(100);

        // Allow the keyboard focus to select the plot
        plotChartPanel.setFocusable(true);

        // Turn off anti-aliasing for the plotted data, and on for the text. This makes the plots
        // draw faster and the lines sharper (e.g., the grid lines are easier to see), and makes
        // the text smoother
        ...
        // Set the min/max drawing height to keep the font from being resized when the chart is
        // resized
        ...
        // Set the x-axis label plot min/max drawing height to keep the font from being resized
        ...
        // Hide the label plot domain axis until the size is determined (when the plot is
        // displayed)
        ...
        plotChartPanel.addKeyListener(new KeyAdapter()
        {
            ...
            public void keyPressed(KeyEvent ke)
            {
                ...
                else if (ke.getKeyCode() == KeyEvent.VK_ESCAPE)
                {
                    ...
                    // Check that the start position exists. This covers the case where the ESC key
                    // is pressed when the plot is first displayed and the mouse hasn't yet moved
                    if (currentPosition != null)
                    {
                        // Clear the start, end, and delta text fields, remove the vertical lines,
                        // and update the start position to match the current cursor position
                ...
                {
                    // Create and send an event to the x-axis scroll bar to block scroll the plot
                ...
                {
                    // Check if the shift and up arrow keys are pressed together
                    if (ke.isShiftDown()
                        && ke.getKeyCode() == KeyEvent.VK_UP)
                    {
                        // Create and send an event to block scroll the plot vertically upwards
                    ...
                    // Check if the shift and down arrow keys are pressed together
                    else if (ke.isShiftDown()
                             && ke.getKeyCode() == KeyEvent.VK_DOWN)
                    {
                        // Create and send an event to block scroll the plot vertically downwards
                        yAxisScrollBar.dispatchEvent(new KeyEvent(yAxisScrollBar,
                                                                  ke.getID(),
                                                                  ke.getWhen(),
                                                                  0,
                    ...
                    // Check if the up, down, page up, or page down arrow key is pressed
    ...
    /**********************************************************************************************
     * Create a listener for mouse button, motion, and wheel changes in the plot
     *********************************************************************************************/
    private void createPlotMouseListener()
    {
        ...
        plotChartPanel.addChartMouseListener(new ChartMouseListener()
        {
            ...
            public void chartMouseClicked(ChartMouseEvent cme)
            {
                ...
                if (chartEntity instanceof PlotEntity)
                {
                    ...
                    // If the Ctrl or Shift key is also pressed then the start and/or end positions
                    // are based on the nearest points to the mouse cursor
                        ...
                        || cme.getTrigger().isShiftDown())
                    {
                        ...
                        {
                            // Determine which subplot the cursor was on when the mouse was clicked
                            // and get the number of points in the plot
                            ps = plotSeries.get(plotChartPanel.getChartRenderingInfo().getPlotInfo().getSubplotIndex(point));
                        }

                        // Only get the start and end positions if the plot has data
                        if (ps.getItemCount() > 0)
                        {
                            // Search for the x-axis value in the plot under the mouse pointer
                            // closest to where the pointer is positioned
                            int dataIndex = findClosestXAxisIndex(ps, currentPosition);

                            // If the Ctrl key is also pressed then set the start and end positions
                            // based on the nearest points to the mouse cursor
                            if (cme.getTrigger().isControlDown())
                            {
                                // Set the new start and end positions, then draw a vertical line
                                // across all plots at these positions and shade the area in
                                // between
                                ...
                                // Update the start, end, and delta fields, and set the mode so
                                // that mouse movement doesn't move the start or end vertical line
                            ...
                            // The SHIFT key is also pressed; set the start (or end, depending on
                            // the cursor mode) position based on the nearest points to the mouse
                            // cursor
                            else
                            {
                                // Use the point that's closest to the mouse position (use the
                                // point to the left if the distances are equal)
                                if (currentPosition - ps.getX(dataIndex - 1).doubleValue() < ps.getX(dataIndex).doubleValue() - currentPosition)
                                {
                                ...
                                // Set the new start (end) position, then draw a vertical line
                                // across all plots at the new positions
                                switch (cursorMode)
                                {
                                    case START:
                                    case DELTA:
                    ...
                    // No relevant modifier keys are also pressed; position the vertical line at
                    // the current mouse position based on the cursor mode and update the
                    ...
                    else
                    {
                        switch (cursorMode)
                        {
                            case START:
                                ...
                                {
                                    // Store the screen x-coordinate of the start position marker.
                                    // This is used to prevent the end position marker from erasing
                                    // the start marker
                                ...
                                // Move the vertical line across all plots to the new start
                                // position
                                adjustStartEndLine(startMarker,
                                                   startPosition,
                                                   true);
            ...
            public void chartMouseMoved(ChartMouseEvent cme)
            {
                ...
                // Only trigger while the mouse cursor is moving through the plot window or if it's
                // over a plotted point
                if (chartEntity instanceof PlotEntity
                    || chartEntity instanceof XYItemEntity)
                {
                    ...
                    {
                        case START:
                            // Store the screen x-coordinate of the start position marker. This is
                            // used to prevent the end position marker from erasing the start
                            // marker
                            startCoord = cme.getTrigger().getPoint().x;
                        ...
                        case END:
                            ...
                            // Move the vertical line across all plots to the new end position. If
                            // the start and end markers have the same screen x-coordinate the new
                            // end marker isn't drawn; otherwise the start marker is erased
                            adjustStartEndLine(endMarker,
                                               endPosition,
                                               startCoord != cme.getTrigger().getPoint().x);
                            break;
                        ...
                        case DELTA:
                            // No lines to move while the delta time is displayed; do nothing
                            break;
                    }
                }
                // Not moving through a plot window (i.e., moving through the label area)
                else
                {
                    // Check if the main application mouse pointer is already selected
        ...
        plotChartPanel.addMouseWheelListener(new MouseWheelListener()
        {
            ...
            {
                // Adjust the x- and y-axis scroll bar unit and block increment values based on the
                // zoom level
    ...
    /**********************************************************************************************
     * Perform a binary search on the x-axis values for the plot under the mouse pointer to find
     * the index into the data set closest to where the pointer is positioned
     *
     * @param ps
     ...
     *
     * @return The index of the point in the plot's data set nearest to the target value
     *********************************************************************************************/
    private int findClosestXAxisIndex(XYSeries ps, double targetValue)
    {
        // Set the initial start and stop indices, ignoring the first and last points
        int lowerIndex = 1;
        ...
        {
            // Set the upper index to the lower index. This prevents the possibility of calculating
            // a negative data index
        ...
        // Continue the search until the plot value matches the mouse pointer position or until the
        // lower and upper indices meet
               ...
               && (lowerIndex <= upperIndex))
        {
            ...
            // Recalculate the midpoint index between the updated upper and lower indices
        ...
        plotChart.addProgressListener(new ChartProgressListener()
        {
            ...
            public void chartProgress(ChartProgressEvent cpe)
            {
                ...
                if (cpe.getType() == ChartProgressEvent.DRAWING_FINISHED)
                {
                    ...
                    // Check if the x-axis scroll bar has changed in width. Also check if the
                    // y-axis label width has changed. This can occur if the plot is zoomed and the
                    // length of the longest visible label changes. If these widths are unchanged
                    // then skip resizing/repositioning the plot window, x-axis scroll bar, and
                    ...
                    if (progressListenerEnabled)
                    {
                        ...
                        // Set the left side x-axis spacing to account for the plot's y-axis
                        // labels; the '- 8' appears to be a fixed amount unaffected by font, L&F,
                        // or tick/label changes
                        space.setLeft(plotYLabelWidth - 8);

                        // Set the right side spacing so that the y-axis scroll bar doesn't overlap
                        // the plots
                        ...
                        // Resize and reposition the x-axis scroll bar by adjusting the border of
                        // the panel containing the scroll bar. The left border width determines
                        // the scroll bar position and is based on the width of the plot's y-axis
                        // labels. The right border width determines the overall scroll bar width
                        // and accounts for the width of the y-axis scroll bar plus its padding
                        ...
                        // Make the scroll bar and label plot visible now that the correct width
                        // has been determined and set
                        xAxisScrollBar.setVisible(true);
                        showLabelDomainAxis(true);

                        // Set up the cursor information fields; this is done here so that the
                        // first field can be aligned with the left edge of the plot windows
                        ...
                        // Must output something in order for the fields to display; use the latest
                        // start position (blank if no position has yet been determined)
                        if (startPosition == null)
                        {
                        ...
                        // Request focus for the plot window so that key presses can be detected.
                        // This call must be made after the plot window is displayed
                        plotChartPanel.requestFocusInWindow();

                        // Disable the chart progress listener so that this section of code won't
                        // repeat every time the vertical lines (domain markers) are moved. This
                        // flag must be reenabled whenever the main or plotting window size is
                        // changed
                        progressListenerEnabled = false;
    ...
    {
        // Once sizes & position are known calculate the height of the label plot window position
        // based on the label plot font sizes and padding attributes
        ...
        // Force the layout manager to redraw the window contents. Without this the plot won't be
        // displayed initially unless the main window is resized
    ...
    {
        // Set up the vertical lines to be displayed for the start/end positions (as determined by
        // the mouse cursor) and set the color and transparency for the shaded area in between
        ...
        {
            // Create a shaded area with a solid border to indicate the data gap
    ...
    {
        // Create the horizontal scroll bar used for scrolling the plots' x-axes
        xAxisScrollBar = new JScrollBar(JScrollBar.HORIZONTAL, 0, 1, 0, 1);

        // Hide the x-axis scroll bar until the plot finishes being drawn. This keeps the scroll
        // bar from showing up initially with the wrong length
        xAxisScrollBar.setVisible(false);

        // Set the scroll bar background color so that its the same as the rest of the plot area
        ...
        xAxisScrollBar.addAdjustmentListener(new AdjustmentListener()
        {
            ...
            {
                // Calculate the new x-axis min/max values and use these to set the domain
        ...
        xAxisScrollBar.addMouseWheelListener(new MouseWheelListener()
        {
            ...
            public void mouseWheelMoved(MouseWheelEvent mwe)
            {
                // Adjust the new x-axis scroll bar thumb position by one unit scroll amount based
                // on mouse wheel rotation direction. The scroll "speed" is reduced by half to make
    ...
    private void createYAxisScrollBar()
    {
        ...
        yAxisScrollBar.addAdjustmentListener(new AdjustmentListener()
        {
            ...
            public void adjustmentValueChanged(AdjustmentEvent ae)
            {
                // Calculate the new y-axis min/max values and use these to set the range
        ...
        yAxisScrollBar.addMouseWheelListener(new MouseWheelListener()
        {
            ...
            public void mouseWheelMoved(MouseWheelEvent mwe)
            {
                // Adjust the new y-axis scroll bar thumb position by one unit scroll amount based
                // on mouse wheel rotation direction
    ...
    /**********************************************************************************************
     * Display markers to show the gaps between individual log files (if multiple files have been
     * opened and concatenated)
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Adjust the position of a start or end line (domain marker) across all plots
     *
     * @param marker
     *            start or end marker
     *
                                    ...
                                    boolean drawNew)
    {
        ...
        // Check if the new marker is to be drawn. If this marker overlays an existing one then
        // this flag should be false to prevent erasing the other marker
        if (drawNew)
        {
            // Draw a vertical line across all plots at the new indicated position
    ...
    /**********************************************************************************************
     * Remove existing start and end lines (domain markers) and the shaded area between the
    ...
    {
        // Erase all of the start and end lines, and the shaded areas on each plot. Don't notify
        // the plot listeners of the changes so as not to send a large number of change events
        for (XYPlot plot : plots)
        {
    ...
    /**********************************************************************************************
     * Reset the start/end/delta text fields, and the start and end lines (domain markers) and
     * shaded areas across all plots
     *
     * @param position
    ...
    private void resetStartEndLines(String position)
    {
        ...
        // Erase the start and end lines (domain markers) and the shaded areas from all of the
        // plots
        clearStartEndLinesAndAreas(shadeMarker, startMarker, endMarker);

        // Move the vertical line across all plots to the new start position. This also sends a
        // plot change event which makes the plot redraw and remove the markers
                                ...
                                Paint lineColor)
    {
        ...
        for (XYPlot plot : plots)
        {
            // Draw a vertical line across all plots at the minor or major frame start position
    ...
    private void hideFrameLines(List<ValueMarker> marker)
    {
        ...
        if (marker != null)
        {
            ...
            for (ValueMarker mkr : marker)
            {
                ...
                for (XYPlot plot : plots)
                {
                    // Remove the minor or major frame line (domain marker), if visible
    ...
    {
        // Set the flags that determine if the minor or major frame lines should be displayed or
        // removed
        ...
        // Hide the minor and major frame lines, if present. This must be done even if the lines
        // are only being changed
        ...
        if (doMinor || doMajor)
        {
            ...
            {
                // Check if this is an entry event. Frame lines are only drawn for entry events
                if (log.isEntryEvent())
                {
    ...
    private double getMouseXCoordinate(Point point)
    {
        ...
        // Convert the mouse x-coordinate to the plot x-coordinate and return the value
    ...
    {
        // The x-axis boundaries are 0.0 (min) and maximumTimeStamp (max); pad these to make the
        // plots look better
    ...
    protected void setYAxisMinMax()
    {
        ...
        for (SymbolAxis sa : symbolAxis)
        {
            ...
            if (yAxisCustomScrollBar)
            {
                // Pad the y-axis boundaries by 1 to make the plots look better. The range
                // difference (upper - lower limit) must be positive, so for the case where no IDs
                // are selected (plotSeries.size() = 1) the upper limit is 1 - 1.499999 =
            ...
            {
                // The y-axis boundaries are 0 (exit) and 1 (entry); pad these to make the plots
                // look better
                sa.setRange(-0.1, 1.1);
            }
        }
    ...
    /**********************************************************************************************
     * Set the horizontal (x-axis) scroll bar thumb size and position based on the current x-axis
     * minimum and maximum values
     *
     * @return Fraction to be displayed of the total plot's x-axis, times the x-axis multiplier
     *********************************************************************************************/
    ...
    /**********************************************************************************************
     * Set the vertical (y-axis) scroll bar thumb size and position based on the current y-axis
     * minimum and maximum values
     *
     * @return Fraction to be displayed of the total plot's y-axis, times the y-axis multiplier
     *********************************************************************************************/
    ...
    {
        // Adjust the x-axis scroll bar unit and block increment values based on the zoom level
        adjustScrollBarIncrements(xAxisScrollBar);

        // Check if the custom y-axis scroll bar is active
        if (yAxisCustomScrollBar)
        {
            // Adjust the y-axis scroll bar unit and block increment values based on the zoom level
            adjustScrollBarIncrements(yAxisScrollBar);
        }
    }

    /**********************************************************************************************
     * Adjust the scroll bar unit and block increment values based on the zoom level
     *
     * @param scrollBar
    ...
    private void adjustScrollBarIncrements(JScrollBar scrollBar)
    {
        ...
        {
            // Adjust the x-axis boundaries if the new values are outside the initial minimum and
            // maximum values or the difference between the maximum and minimum is below the
            // minimum allowed
            ...
            // Adjust the x-axis scroll bar thumb size and position and get the fraction of the
            // total plot that's visible
            fraction = setXAxisScrollBar();
        }
        // Adjust the y-axis scroll bar
        else
        {
            // Adjust the y-axis boundaries if the new values are outside the initial minimum and
            // maximum values or the difference between the maximum and minimum is below the
            // minimum allowed
            ...
            // Adjust the y-axis scroll bar thumb size and position and get the fraction of the
            // total plot that's visible
            fraction = setYAxisScrollBar();
        }

        // Calculate the block and unit increments based on the new range. Each block scroll is
        // approximately the current span of the the x-axis (slightly less so that some of the
        // previous plot remains visible), and each unit scroll is 1/30 of the block scroll. The
        // scroll values are set to 1 if the calculated values equal zero so that scrolling is
        // still possible no matter how far the plot is zoomed in
    ...
    public class CPMChartPanel extends ChartPanel
    {
        ...
        /******************************************************************************************
         * Override the context menu Save As command. The chart x-axis label and tick labels are
         * normally disabled; these are displayed in a separate (label) chart, which is necessary
         * for the combined step plots embedded in a scroll pane so that the labels can't scroll
         * out of view. The chart labels are enabled briefly so that the saved version of the plot
        ...
        public void doSaveAs()
        {
            try
            {
                // Enable the "actual" x-axis label display. For screen display a separate chart is
                // used for displaying the x-axis labels
        ...
        /******************************************************************************************
         * Override the context menu Copy command. The chart x-axis label and tick labels are
         * normally disabled; these are displayed in a separate (label) chart, which is necessary
         * for the combined step plots embedded in a scroll pane so that the labels can't scroll
         * out of view. The chart labels are enabled briefly so that the copy of the plot will
        ...
        /******************************************************************************************
         * Override the context menu Print command. The chart x-axis label and tick labels are
         * normally disabled; these are displayed in a separate (label) chart, which is necessary
         * for the combined step plots embedded in a scroll pane so that the labels can't scroll
         * out of view. The chart labels are enabled briefly so that the printed of the plot will
         * display these labels
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * Force the plot x-axis boundaries to their initial values when unzooming
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * Force the plot y-axis boundaries to their initial values when unzooming
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * This is called when using the context menu Zoom In Domain Axis command. Adjust the
         * x-axis scroll bar thumb size and position when zooming in
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * This is called when using the context menu Zoom Out Domain Axis command. Adjust the
         * x-axis scroll bar thumb size and position when zooming out
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * This is called when using the context menu Zoom In Range Axis command. If the custom
         * y-axis scroll bar is in use (i.e., for the thread plot) adjust the y-axis scroll bar
         * thumb size and position when zooming in. If the custom y-axis scroll bar is not in use
         * (i.e., for the step plot) force the plot y-axis boundaries to their initial values
         *****************************************************************************************/
        @Override
        public void zoomInRange(double x, double y)
        {
            ...
            {
                // Adjust the y-axis scroll bar unit and block increment values based on the zoom
                // level
                adjustScrollBarIncrements(yAxisScrollBar);
            }
        }
        ...
        /******************************************************************************************
         * This is called when using the context menu Zoom Out Range Axis command. If the custom
         * y-axis scroll bar is in use (i.e., for the thread plot) adjust the y-axis scroll bar
         * thumb size and position when zooming out. If the custom y-axis scroll bar is not in use
         * (i.e., for the step plot) force the plot y-axis boundaries to their initial values
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * This is called when using the context menu Zoom In Both Axes command. Adjust the x- and
         * y-axis scroll bar thumb sizes and position when zooming in
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * This is called when using the context menu Zoom out Both Axes command. Adjust the x- and
         * y-axis scroll bar thumb sizes and position when zooming out
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * Remove the active domain marker (start or end) and clear its text field when the mouse
         * cursor exits the plot
         *****************************************************************************************/
        @Override
        ...
        /******************************************************************************************
         * Create a tool tip by converting the screen X-Y coordinates into chart area X-Y
         * coordinates and then looks for a data point in a data set that lies within a "hot spot"
         * bounding box around that point. Overriding the method is necessary to improve the
         * execution speed, otherwise the plot updates very slowly with tool tips enabled
         *****************************************************************************************/
        @Override
        public String getToolTipText(MouseEvent me)
        {
            ...
            if (toolTipText != null)
            {
                ...
                if (plot instanceof CombinedDomainXYPlot)
                {
                    ...
                    // Check if the pointer is over a subplot and not the gap between subplots. If
                    // the pointer is over a gap then the range axis will return null below and the
                    // tool tip check is skipped
                    if (index >= 0)
                    {
                ...
                // Check if the plot has a valid range axis. The label plot, used to display the
                // x-axis labels, does not have a range axis and can be skipped
                if (rangeAxis != null)
                {
                    ...
                    // Calculate the boundaries of the current pointer position based on the hot
                    // spot size
                    ...
                    // Initialize the flag that indicates if a note has been found
                    ...
                    for (int series = 0; series < seriesCount && !foundPoint; series++)
                    {
                        ...
                        for (int item = 0; item < itemCount && !foundPoint; item++)
                        {
                            ...
                            // Check if the data set 'X' value lies between the hot spot boundaries
                            // (xLowerBound < xValue < xUpperBound)
                            if (xLowerBound < xValue)
                            {
                                if (xValue < xUpperBound)
                                {
                                    ...
                                    // Check if the data set 'Y' value lies between the hot spot
                                    // boundaries (yLowerBound < yValue < yUpperBound)
                                    if (yLowerBound < yValue && yValue < yUpperBound)
                                    {
                                        // Point is within the hot spot boundaries; get the overrun
                                        // and/or annotation associated with the point
                                        ...
                                        // Set the flag indicating a point was found in order to
                                        // exit the loops
                                        foundPoint = true;
                                    }
                                }
                                // The x-values only get larger, so no need to keep checking once
                                // the maximum possible for this tool tip is exceeded
                                else
                                {
                                    break;
                                }
        ...
        /******************************************************************************************
         * Append an overrun and/or an annotation, if these exists, to the tool tip text string
         * associated with a specified point. Apply word wrapping to the note to prevent making the
         * tool tip box too wide
         *
         * @param plot
                                       ...
                                       int item)
        {
            ...
            for (PerfLog log : perfLog.getPerfLogData())
            {
                // Check if this log entry is the one under the mouse pointer (i.e., the time stamp
                // matches the selected point and the series key matches the ID name)
                    ...
                    && dataset.getSeriesKey(series).equals(perfIDs.getPerfID(log.getPerfID()).getPerfIDName(true)))
                {
                    ...
                    if (!log.getNotes().isEmpty())
                    {
                        ...
                        // Process the note to break it into pieces for word wrapping purposes
                        ...
                        // Check if any text is remains after breaking the note into pieces
                        if (text.length() != 0)
                        {
        ...
        /******************************************************************************************
         * Search for a space within a string starting at a predefined break point and in the
         * specified direction. Append the portion of the string up to that point to the tool tip
         * text string
         *
         * @param text
         ...
         *
         * @param direction
         *            direction in which to search (-1 for backwards, 1 for forwards)
         *
                                   ...
                                   int direction)
        {
            ...
            // Step backwards through the string starting at the target wrap point
    ...
    /**********************************************************************************************
     * Check the axis lower and upper boundary values and, if the value is outside the specified
     * limits then restore the boundary to the limit value. Check the difference between the upper
     * and lower boundary values and, if the difference is below a specified minimum, adjust the
     ...
     *
     * @param minimumDelta
     *            minimum allowed difference between the upper and lower boundary values
     *********************************************************************************************/
    private void limitAxisBoundaries(ValueAxis axis,
                                     ...
                                     double minimumDelta)
    {
        // Check if the lower boundary value is less than the minimum lower boundary value
        ...
        // Check if the upper boundary value is less than the maximum upper boundary value
        ...
        // Check if the difference between the upper and lower boundary values is less than the
        // minimum allowed
        if (axis.getUpperBound() - axis.getLowerBound() < minimumDelta)
        {
            ...
            else
            {
                ...
                // Set the lower and upper values equidistant from the midpoint with a difference
                // equal to the minimum allowed
    ...
    private void createPlotDomainAxes()
    {
        ...
        // Set the x-axis tick label format and the number of minor tick lines between each labeled
        // tick mark
    ...
    /**********************************************************************************************
     * Show or hide the vertical grid lines based on the user's selection in the plot Preferences
     * dialog
     *********************************************************************************************/
    ...
    {
        // Create a series for the x-axis label plot and populate it with the min/max x values. The
        // y values aren't used for the label plot
    ...
    /**********************************************************************************************
     * Show or hide the label domain axis (x-axis) components by setting the colors. This allows
     * the domain axis to be displayed, but hidden, until the size can be determined, at which
     * point the colors are set to show the domain axis components
     *
     * @param show
    ...
    /**********************************************************************************************
     * Set the plot background and grid line colors based on the plot Preference dialog selections
     *
     * @param adjustFirstColor
     *            true if the first line is not a performance ID (e.g. the CPU activity or thread
     *            of execution lines)
     *********************************************************************************************/
    protected void setPlotColors(boolean adjustFirstColor)
    {
        ...
        // Check if the first line's color needs to be determined. This is true if the first line
        // represents the CPU activity
        if (adjustFirstColor)
        {
            // Set the RGB value to fully off (0) or fully on (255) based on the combination of the
            // background color's red, green, and blue components. A dark background results in a
            // white line color; a light background results in a black line color
    ...
    /**********************************************************************************************
     * Add or remove frame overrun indicators based on the plot Preference dialog selection
     *********************************************************************************************/
    protected void showOverruns()
    {
        ...
        // Check if this is the step plot and if more than one subplot is displayed
        if (plotType == STEP_PLOT && plotSeries.size() != 1)
        {
            // Set the index to the first ID plot (skip the CPU idle plot, if present)
            index = 1;
        }
        ...
        for (PerformanceID pid : perfIDs.getPerfIDList())
        {
            ...
            if (pid.isPlotPerfID())
            {
                ...
                if (plotPrefs.isShowOverruns(false))
                {
                    ...
                    for (PerfLog log : perfLog.getPerfLogData())
                    {
                        // Check if logged ID matches the series ID and if an overrun condition
                        // exists
