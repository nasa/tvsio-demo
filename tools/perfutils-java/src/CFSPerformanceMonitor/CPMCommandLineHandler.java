...
public class CPMCommandLineHandler
{
    ...
    private class CommandHandler
    {
         ...
         *****************************************************************************************/
        protected CommandHandler(String command, CmdType type, Object[] conditions)
        {
            this.command = command;
            this.type = type;
                          ...
                          final CPMPreferencesHandler plotPrefs)
    {
        ...
        // Check if an invalid command argument or parameter was provided, or if the parameter was
        // missing
        if (errorFlag)
        {
     ...
     *
     * @return Selected performance log file(s). If more than one file is listed then the path+file
    ...
    private Integer handleID(String arg, String parm)
    {
        ...
        if (parm.length() <= 8 && parm.matches("[0-9A-Fa-f]+"))
        {
            ...
            // Check that the high bit isn't set. The high bit is reserved for indicating exit
            // events
            if ((val | EXIT_MASK) != 0)
            {
