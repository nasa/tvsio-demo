...
public class CPMConstants
{
    ...
    protected static final String CPM_VERSION = "1.0.2";
    protected static final String CPM_DATE = "January 2019";
    ...
    // 'Magic number' for cFE compliant files defined in cfe_fs.h. The hexadecimal equivalent to
    // 'cFE1'. A log file is identified by beginning with this pattern
    ...
    // Source code subfolders to search for performance IDs. The text case of the names in this
    // list don't affect the search since both this name and the comparison folder name are forced
    // to lower case prior to comparing the names
    ...
    // Only text in the targeted source code ending with this sequence are considered performance
    // IDs
    ...
    // Masks to check for or remove the high bit, which (if set) indicates this is an exit event
    // (otherwise it's an entry event)
    protected static final long ID_MASK = 0x7fffffffL;
    protected static final long EXIT_MASK = 0x80000000L;

    // Log file name separator in the program preferences backing store if multiple log files are
    // stored
    ...
    // Dialog box button padding, in pixels, between the group of buttons and the edge of the
    // dialog window or upper dialog components)
    ...
    // Horizontal and vertical spacing, in pixels, between a text label and another component
    protected static final int LABEL_HORIZONTAL_SPACING = 10;
    protected static final int LABEL_VERTICAL_SPACING = 7;

    // Check box horizontal spacing, in pixels, between the left side of the dialog and the check
    // box
    protected static final int CHECK_BOX_HORIZONTAL_SPACING = 6;

    // Padding to the bottom (x-axis) or right side (y-axis) of the plot scroll bars, in pixels
    protected static final int PLOT_SCROLL_BAR_PAD = 4;

    // Padding between the bottom of the plot and the x-axis label plot, in pixels
    protected static final int PLOT_XAXIS_LABEL_PAD = 2;

    // Table header horizontal and vertical padding, in pixels. The horizontal padding provides
    // room for the column sort arrow
    ...
    // Calculate the minimum time delta displayed in a plot based on the time precision
    protected static double MINIMUM_TIME_DELTA = Math.pow(10.0,
                                                          -TIME_PRECISION);

    // Format for the step and thread of execution plot, and bar chart x-axis tick labels
    ...
    // Set the plot horizontal (x-axis) and vertical (y-axis) scroll bar fineness multipliers (a
    // higher value allows for finer control of the scroll bar as the plot is zoomed in)
    protected static final int X_SCROLLBAR_MULT = 100000000;
    protected static final int Y_SCROLLBAR_MULT = 10000;

    // Bias, in pixels, to align the statistics and totals tables in the statistics dialog. The
    // value is dependent on the look & feel
    protected static int LAF_BIAS = 0;

    // Used in the ID editor when rendering the desired background color for the Show cell (check
    // boxes). The value is dependent on the look & feel, and if not set correctly the background
    // color may be wrong
    ...
    // Size of a check box icon (width and height) in pixels. Used to size the color selection
    // check boxes in the Preferences dialog
    ...
    {
        // Based on the selected look & feel, adjust the bias that's needed to align the two tables
        // in the statistics dialog, and set the opaque flag used to adjust the rendering of the
        // check boxes in the ID table
        ...
        // Determine the height, in pixels, of a check box icon. The correct height is not returned
        // by the UI manager for all look & feels unless the check box is first realized, so an
        // invisible window is created, a check box instantiated, then the window removed. The
        // check box size returned by the UI manager is now correct
