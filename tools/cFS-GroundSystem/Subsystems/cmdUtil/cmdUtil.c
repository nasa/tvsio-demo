...
/* using CCSDS_VER_2 as message format */
#ifdef MESSAGE_FORMAT_IS_CCSDS_VER_2
    #define PACKETHDR_SIZE    (sizeof(short) * 6)
#else
...
void ProcessStringArgument(char *optarg, CommandData_t *CommandData) {
    ...
    stringLength = strtol(stringLenString, NULL, 10);
        ...
        printf("String is '%s'\n", (char*)&(optarg[stringIndex]));
        printf("String Length is %d.\n", stringLength);
        printf("CommandData->DataLen %d.\n", CommandData->DataLen);
    }
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    while( opt != -1 )
    {
        ...
        case 'I':
            ...
            tempShort = (short)strtol(optarg, 0, 0);
                ...
                short streamID0 = 0x38;
                short streamID1 = 0x00FF & tempShort;
                if (hostByteOrder != LITTLE_ENDIAN ){
                    byteSwap((char*)&streamID0, sizeof(streamID0));
                    byteSwap((char*)&streamID1, sizeof(streamID1));
                    // streamID1 = 0xFF00 & tempShort;
                }
                short abidSubsystem0 = 0x00;

                // for command IDS starting with 0x8E*
                // short abidSubsystem1 = (tempShort & 0x0F00) >> 9;

                // for command IDS starting with 0x9E*
                short abidSubsystem1 = 0xF;
                short abidSystem0 = 0x00;
                short abidSystem1 = 0x00;

                if (hostByteOrder != LITTLE_ENDIAN ){
                    memcpy(&CommandData.PacketHdr[1], &streamID0, sizeof(streamID0));
                    memcpy(&CommandData.PacketHdr[0], &streamID1, sizeof(streamID1));
                    memcpy(&CommandData.PacketHdr[7], &abidSubsystem0, sizeof(abidSubsystem0));
                    memcpy(&CommandData.PacketHdr[6], &abidSubsystem1, sizeof(abidSubsystem1));
                    memcpy(&CommandData.PacketHdr[9], &abidSystem0, sizeof(abidSystem0));
                    memcpy(&CommandData.PacketHdr[8], &abidSystem1, sizeof(abidSystem1));
                }else{
                    memcpy(&CommandData.PacketHdr[0], &streamID0, sizeof(streamID0));
                    memcpy(&CommandData.PacketHdr[1], &streamID1, sizeof(streamID1));
                    memcpy(&CommandData.PacketHdr[6], &abidSubsystem0, sizeof(abidSubsystem0));
                    memcpy(&CommandData.PacketHdr[7], &abidSubsystem1, sizeof(abidSubsystem1));
                    memcpy(&CommandData.PacketHdr[8], &abidSystem0, sizeof(abidSystem0));
                    memcpy(&CommandData.PacketHdr[9], &abidSystem1, sizeof(abidSystem1));
                }
            #else
                /* 
	    ...
	    tempShort <<= 8;
            ...
            */
            ...
            #ifdef MESSAGE_FORMAT_IS_CCSDS_VER_2
                memcpy(&CommandData.PacketHdr[10], &tempShort, sizeof(tempShort));
            #else
    ...
    /* four extra bytes were added to the header data thus increating the packet data length */
    #ifdef MESSAGE_FORMAT_IS_CCSDS_VER_2
        CommandData.DataLen += 4;
    #endif
