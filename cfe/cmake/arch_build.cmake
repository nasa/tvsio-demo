...
function(add_cfe_tables APP_NAME TBL_SRC_FILES)
  ...
  foreach(TBL ${TBL_SRC_FILES} ${ARGN})
    ...
    foreach(TGT ${APP_INSTALL_LIST})
      ...
      unset(TBL_SRC CACHE)
      find_file(TBL_SRC 
                NAMES ${TGT}_${TBLWE}.c ${TBLWE}.c
                PATHS ${MISSION_DEFS}/${TGT}/${APP_NAME}/tables ${MISSION_DEFS}/${TGT}/${APP_NAME}
                      ${MISSION_DEFS}/${TGT}/tables ${MISSION_DEFS}/${TGT}
                      ${MISSION_DEFS}/tables ${MISSION_DEFS}
                      ${MISSION_SOURCE_DIR}/tables/${TGT}/${APP_NAME} ${MISSION_SOURCE_DIR}/tables/${TGT}
                      ${MISSION_SOURCE_DIR}/tables
                      NO_DEFAULT_PATH)
      if (${TBL_SRC} STREQUAL "TBL_SRC-NOTFOUND")
         if (IS_ABSOLUTE "${TBL}")
           set(TBL_SRC "${TBL}")
         else()
      ...
      if (NOT EXISTS "${TBL_SRC}")
        ...
        message(STATUS "Target \"${TGT}\" : \n     Selected file ${TBL_SRC} as source for ${TBLWE}")
      endif()    
...
function(prepare)
  ...
  #generate_config_includefile("inc/osconfig.h" osconfig.h ${TARGETSYSTEM}/${TARGETSYSTEM} ${TARGETSYSTEM}/${OSAL_SYSTEM_OSCONFIG} 
  #                             shared/${TARGETSYSTEM} ${TARGETSYSTEM} shared/${OSAL_SYSTEM_OSCONFIG} ${OSAL_SYSTEM_OSCONFIG})
  generate_config_includefile("inc/osconfig.h" osconfig.h ${TARGETSYSTEM}/cfgs/${TARGETSYSTEM} )
  message("")
...
function(process_arch SYSVAR)
  ...
  foreach(TGTID ${TGTSYS_${SYSVAR}})
    ...
    # Copy apps' mission header file
    foreach(APP ${TGT${TGTID}_APPLIST})
       set (AFILE ${MISSION_DEFS}/shared/cfgs/${APP}_mission_cfg.h)
       if (EXISTS "${AFILE}")
            generate_config_includefile("inc/${APP}_mission_cfg.h" mission_cfg.h shared/cfgs/${APP})
       endif (EXISTS "${AFILE}")
    endforeach(APP ${TGT${TGTID}_APPLIST})
    message("")

      # Generate wrapper file for the requisite cfe_platform_cfg.h file
      list(REVERSE TGTPLATFORM)
      list(APPEND TGTPLATFORM "cfe")
      set(TGT_SRC_PATHS_TEMP ${TGTNAME}/cfgs ${TGTNAME} shared)
      foreach(TGT_SRC_PATH_ITEM ${TGT_SRC_PATHS_TEMP})
         foreach(TGTPLATFORMITEM ${TGTPLATFORM})
            list(APPEND TGT_SRC_PATHS ${TGT_SRC_PATH_ITEM}/${TGTPLATFORMITEM})
         endforeach(TGTPLATFORMITEM ${TGTPLATFORM})
      endforeach(TGT_SRC_PATH_ITEM ${TGT_SRC_PATHS_TEMP})
      foreach(TGTPLATFORMITEM ${TGTPLATFORM})
         list(APPEND TGT_SRC_PATHS ${TGTPLATFORMITEM})
      endforeach(TGTPLATFORMITEM ${TGTPLATFORM})
      #generate_config_includefile("${CFE_CORE_TARGET}/inc/cfe_msgids.h" msgids.h ${TGT_SRC_PATHS})
      #generate_config_includefile("inc/cfe_platform_cfg.h" platform_cfg.h ${TGT_SRC_PATHS})
      generate_config_includefile("inc/cfe_platform_cfg.h" platform_cfg.h ${TARGETSYSTEM}/cfgs/cfe)

      # Install mission-specific or CPU-specific headers in build/cpuX/inc.
      # Priority is given to CPU-specific over mission-specific headers.
      # Install any other CPU-specific headers in build/cpuX/inc
      # A mission-specific header simply needs to exist in the defs directory.
      # A CPU-specific header is assumed to be named cpuX_foo.h and will be installed as foo.h
      # Exceptions are made for cfe_msgids.h and cfe_platform_cfg.h, where the leading "cpuX" 
      # is replaced by "cfe".      
      set(EXCLUSION_NAMES "mission_cfg.h" "perfids.h" "osconfig.h")
      foreach(TGTPLATFORMITEM ${TGTPLATFORM})
         list(APPEND EXCLUSION_NAMES "${TGTPLATFORMITEM}_platform_cfg.h")
         #list(APPEND EXCLUSION_NAMES "${TGTPLATFORMITEM}_msgids.h")
      endforeach(TGTPLATFORMITEM ${TGTPLATFORM})

      FILE(GLOB ALLHEADERS ${MISSION_DEFS}/${TGTNAME}/*.h* ${MISSION_DEFS}/${TGTNAME}/cfgs/*.h* ${MISSION_DEFS}/shared/*.h* ${MISSION_DEFS}/*.h*)
      set(TGTHEADERS ${ALLHEADERS})
      list(REMOVE_DUPLICATES TGTHEADERS)
      foreach(TESTHEADER ${ALLHEADERS})
         get_filename_component(HEADERNAME ${TESTHEADER} NAME)
         set(ISEXCL 0)
         foreach(EXCLUSION_NAME ${EXCLUSION_NAMES})
            string(FIND ${HEADERNAME} ${EXCLUSION_NAME} RES)
            if(NOT ${RES} EQUAL -1)
               list(REMOVE_ITEM TGTHEADERS ${TESTHEADER})
               set(ISEXCL 1)
            endif()
         endforeach(EXCLUSION_NAME ${EXCLUSION_NAMES})
         if(${ISEXCL} EQUAL 0)
            STRING(FIND ${HEADERNAME} "cpu" ISCPU)
            STRING(FIND ${HEADERNAME} ${TGTNAME} ISTARGET)
            if(${ISCPU} EQUAL 0)
               if(${ISTARGET} EQUAL 0)
                  STRING(REGEX REPLACE "^${TGTNAME}_" "" PROCESSEDHEADERNAME ${HEADERNAME})
                  list(REMOVE_ITEM TGTHEADERS ${MISSION_DEFS}/${PROCESSEDHEADERNAME})
               else()
                  list(REMOVE_ITEM TGTHEADERS ${TESTHEADER})
               endif()
            endif()
      ...
      foreach (TGTHEADER ${TGTHEADERS})
         get_filename_component(HEADERNAME ${TGTHEADER} NAME)
         STRING(REGEX REPLACE "^${TGTNAME}_" "" PROCESSEDHEADERNAME ${HEADERNAME})
         SET(INSTALLPATH "inc/${PROCESSEDHEADERNAME}")
         STRING(FIND ${PROCESSEDHEADERNAME} "_" SUBIDX)
         STRING(SUBSTRING ${PROCESSEDHEADERNAME} 0 ${SUBIDX} PREFIX)
         math(EXPR SUBIDX "${SUBIDX} + 1")
         STRING(SUBSTRING ${PROCESSEDHEADERNAME} ${SUBIDX} -1 SUFFIX)
         generate_config_includefile(${INSTALLPATH} ${SUFFIX} ${TGTNAME}/cfgs/${PREFIX} ${TGTNAME}/${PREFIX} shared/${PREFIX} ${PREFIX})
      endforeach()
      message("")
    ...
    foreach(INSTFILE ${TGT${TGTID}_FILELIST})
      unset(FILESRC CACHE)
      find_file(FILESRC
                NAMES ${TGTNAME}_${INSTFILE} ${INSTFILE}
                PATHS ${MISSION_DEFS}/${TGTNAME} 
                      ${MISSION_DEFS}
                      NO_DEFAULT_PATH)

      if(${FILESRC} STREQUAL "FILESRC-NOTFOUND")
        ...
        message(STATUS "Target \"${TGTNAME}\" : \n     Selected file ${FILESRC} to be installed at ${INSTALL_SUBDIR}/${INSTFILE}")
      endif()
