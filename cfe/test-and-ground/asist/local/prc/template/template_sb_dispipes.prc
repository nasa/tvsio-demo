...
;	06/03/2005   Ingrid Albright	Original Procedure
;       12/04/2006   Ingrid Albright	Updated from new scenario. Now a 
;					combination of SB Pipes and SB Loading.
;	02/20/2007   Walt Moleski	Removed requirements not tested in this
;					procedure.
;	07/05/2007   Walt Moleski	Removed deleted rqmts (4006;4005.2)
;	08/01/2007   Walt Moleski	Added tests for Rqmts 4003.1,4008, &
;					4008.1. Steps 1.4 thru 1.6 were added.
;       10/26/2017   Walt Moleski	Updated for 6.6.0 testing using CPU1 for
;					commanding and added a hostCPU variable
;					for the utility procs that connect to
;					the host IP. Updated macro names.
;
;  Arguments
;	Name			Description
;	None
...
for i=1 to CFE_PLATFORM_SB_MAX_MSG_IDS do
...
for i=1 to CFE_PLATFORM_SB_MAX_MSG_IDS do
   if ($sc_$CPU_SB_RouteEntry[i].SB_MSGID = stream) then
      ...
      i=CFE_PLATFORM_SB_MAX_MSG_IDS+1
   endif
   if (i=CFE_PLATFORM_SB_MAX_MSG_IDS) then
...
write "*** Pipe Depth"
write "***  Max Allowed: ", $SC_$CPU_SB_STAT.SB_SMMPDALW
for i=1 to CFE_PLATFORM_SB_MAX_PIPES do
...
write ";**********************************************************************"
write ";  Step 9.4: Send CFE_PLATFORM_SB_MAX_PIPE_DEPTH messages to pipe 4."
write ";**********************************************************************"
local msgCtrCurrent
...
local logresumepoint = CFE_PLATFORM_SB_MAX_PIPE_DEPTH-2
...
for i=(logresumepoint+1) to CFE_PLATFORM_SB_MAX_PIPE_DEPTH+1 do
   write "*** ",i,"."
   /$SC_$CPU_TST_SB_SendMsg2
   ut_tlmwait $SC_$CPU_TST_SB_P0CMDPC {sbtstcmdpc+i} 10
   ; Account for the rollover that appears to happen.
...
if ($SC_$CPU_SB_RouteEntry[pipe4index].SB_MSGCNT < CFE_PLATFORM_SB_MAX_PIPE_DEPTH) then
   write "<!> Failed (4003;4706) - incorrect pipe message counter. Expected ",CFE_PLATFORM_SB_MAX_PIPE_DEPTH," got ", $SC_$CPU_SB_RouteEntry[pipe4index].SB_MSGCNT, "."
   ut_setrequirements cSB4706, "F"
   ut_setrequirements cSB4003, "F"
elseif ($SC_$CPU_SB_RouteEntry[pipe4index].SB_MSGCNT > CFE_PLATFORM_SB_MAX_PIPE_DEPTH) then
   write "<!> Failed (4706) - Pipe 4 has accepted more than ",CFE_PLATFORM_SB_MAX_PIPE_DEPTH," messages without overflowing."
   ut_setrequirements cSB4706, "F"
elseif ($SC_$CPU_SB_RouteEntry[pipe4index].SB_MSGCNT = CFE_PLATFORM_SB_MAX_PIPE_DEPTH) then
   write "<*> Passed - Pipe 4 has accepted ",CFE_PLATFORM_SB_MAX_PIPE_DEPTH," messages."
endif
...
write ";            CFE_PLATFORM_SB_MAX_PIPE_DEPTH pipe messages are received."
write ";**********************************************************************"
/$SC_$CPU_TST_SB_EnablePipe

ut_tlmwait $SC_$CPU_TST_SB_MSGCTR {msgCtrCurrent+CFE_PLATFORM_SB_MAX_PIPE_DEPTH} 60

write "*** Message counter: ", $SC_$CPU_TST_SB_MSGCTR, " expected ", (msgCtrCurrent+CFE_PLATFORM_SB_MAX_PIPE_DEPTH), "."
...
local msgidsleft = CFE_PLATFORM_SB_MAX_MSG_IDS
local prevmsgid = 0
local lastMsgId = 0
...
if (numFound = (4+CFE_PLATFORM_SB_MAX_DEST_PER_PKT)) then
   ut_setrequirements cSB4500, "F"
