...
local previous_event_type_status[CFE_PLATFORM_ES_MAX_APPLICATIONS]
local current_event_type_status[CFE_PLATFORM_ES_MAX_APPLICATIONS]
local event_type_status_flag[CFE_PLATFORM_ES_MAX_APPLICATIONS]

local previous_app_msg_sent_ctr[CFE_PLATFORM_ES_MAX_APPLICATIONS]
local current_app_msg_sent_ctr[CFE_PLATFORM_ES_MAX_APPLICATIONS]
local msg_ctr_status_flag[CFE_PLATFORM_ES_MAX_APPLICATIONS]

local previous_port_mask
local port_mask_status_flag

local previous_bin_fltr_ctr[CFE_PLATFORM_ES_MAX_APPLICATIONS]
local current_bin_fltr_ctr[CFE_PLATFORM_ES_MAX_APPLICATIONS]
...
write ";***********************************************************************"
write "; Step 2.1: Send a noop command with an invalid length."
write ";***********************************************************************"
;; Setup to capture event
ut_setupevents $SC, $CPU, CFE_EVS, CFE_EVS_LEN_ERR_EID, ERROR, 1

local errcnt = $SC_$CPU_EVS_CMDEC + 1

;; Send an invalid length command
local rawcmd = "1801C000000200"

ut_sendrawcmd "$SC_$CPU_EVS", (rawcmd)

ut_tlmwait $SC_$CPU_EVS_CMDEC {errcnt}
IF (UT_TW_status = UT_Success) THEN
  write "<*> Passed - Invalid length command failed as expected"
else
  write "<!> Failed - Invalid length command did not increment the error counter"
endif
...
if ($SC_$CPU_find_event[1].num_found_messages = 1) then
  ...
  write "<!> Failed - Event msg ",$SC_$CPU_evs_eventid," received. Expected event msg ",CFE_EVS_LEN_ERR_EID
endif
