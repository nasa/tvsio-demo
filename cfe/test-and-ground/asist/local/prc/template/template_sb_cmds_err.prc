...
;	10/26/17	Walt Moleski	Updated for cFE 6.6. Removed Step 2b
;					since the MAX_MSG test is sent in the
;					sb_enapipes test.
;
;  Arguments
;
...
write ";**********************************************************************"
write ";  Step 1a:  Send a command with an invalid command code. "
write ";**********************************************************************"
...
write ";**********************************************************************"
write ";  Step 1b:  Send a NOOP command with an invalid length. "
write ";**********************************************************************"
ut_setupevents "$SC", "$CPU", "CFE_SB", CFE_SB_LEN_ERR_EID, "ERROR", 1

rawcmd = "1803C000000500" 
...
;  Look for the event message
  if ($SC_$CPU_find_event[1].num_found_messages > 0) then
    ...
    write "<!> Failed - Event Message not received for Invalid command length command."
  endif
else
   write "<!> Failed - Invalid command length test failed."
endif
...
write ";           CFE_MISSION_SB_MAX_SB_MSG_SIZE bytes + 1 (= 32769 bytes).   "
...
;; cFE 6.6.0 commented this step out since it is sent in sb_enapipes from the
;; TST_SB application - WFM
;;write ";**********************************************************************"
;;write ";  Step 2b:  Send a raw command with a packet size of                  "
;;write ";           CFE_MISSION_SB_MAX_SB_MSG_SIZE bytes (= 32769 bytes).      "
