...
FOR apps = 1 to CFE_MISSION_ES_MAX_APPLICATIONS DO
...
;; Setup to receive the SHORT Event Packet
local stream = x'809'
local seqCntItem = "p009scnt"

/$SC_$CPU_TO_ADDPACKET Stream=stream Pkt_Size=x'0' Priority=x'0' Reliability=x'1' Buflimit=x'4'
wait 5

local currSeqCnt = {seqCntItem}
local expSeqcnt = currSeqCnt + 4

/$SC_$CPU_TST_EVS_SendEvtMsg ERROR EventId="1" Iters="4" MILLISECONDS = "150"
wait 15

;; cFE 6.6.0 sends a new packet for SHORT Event Messages This test will just
;; capture that packet and verify the sequence count is +4
if ({seqCntItem} = expSeqcnt) then
  write "<*> Passed (3201) - Event packets for short format were received"
  ut_setrequirements cEVS3201, "P"
else  
  write "<!> Failed (3201) - Incorrect number of Short Event packets rcv'd; Got ",{seqCntItem}, "; Expected ",expSeqcnt
  ut_setrequirements cEVS3201, "F"
endif

;; Prior to cFE 6.6.0 the code below was used
;;if ($sc_$cpu_find_event[1].num_found_messages <> 4) then
;;  write "<!> Failed - Wrong number of evt msgs found. Expected 4; Got ",$sc_$cpu_find_event[1].num_found_messages
;;else 
;;  if ($sc_$cpu_find_event[1].event_txt = "") then
...
FOR i = 1 to CFE_MISSION_ES_MAX_APPLICATIONS DO
...
if ($sc_$cpu_EVS_LOGMODE = CFE_PLATFORM_EVS_DEFAULT_LOG_MODE) then
  ut_setrequirements cEVS3203, "P"
else
  ut_setrequirements cEVS3203, "P"
  write "<!> Failed (3203) - Default Logging Mode. Expected ",CFE_PLATFORM_EVS_DEFAULT_LOG_MODE,"; Got ", $sc_$cpu_EVS_LOGMODE
