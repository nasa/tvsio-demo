...
;*******************************************************************************
; If CFE_PLATFORM_EVS_LOG_ON configuration parameter is not defined then exit proc with
; appropriate message.
;*******************************************************************************
...
; CFE_PLATFORM_EVS_LOG_ON configuration parameter is defined.
...
if (CFE_PLATFORM_EVS_LOG_MAX > 0) then
   ut_setrequirements cEVS3301, "P"
   ;;write "<*> Passed (3301) - Max log entries is set to ",CFE_EVS_LOG_MAX
   write "<*> Passed (3301) - Max log entries is set to ",CFE_PLATFORM_EVS_LOG_MAX
else
...
for app_index = 1 to CFE_PLATFORM_ES_MAX_APPLICATIONS do
...
FOR i = 1 to CFE_PLATFORM_EVS_LOG_MAX DO
...
write ";**********************************************************************"
write "; Step 3.1: Send CFE_PLATFORM_EVS_LOG_MAX - 1 Event Messages"
write ";**********************************************************************"
;            Verify that:
;               p@$SC_$CPU_EVS_LOGFULL = FALSE
;               $SC_$CPU_EVS_LOGOVERFLOWC = 0
...
/$SC_$CPU_TST_EVS_SendEvtMsg INFO EventId = "7" Iters = CFE_PLATFORM_EVS_LOG_MAX-1 Milliseconds = 15
;;wait CFE_EVS_LOG_MAX+1
wait CFE_PLATFORM_EVS_LOG_MAX+1

;;evs_msg_sent_counter = CFE_EVS_LOG_MAX - 1
evs_msg_sent_counter = CFE_PLATFORM_EVS_LOG_MAX - 1
...
write ";**********************************************************************"
write "; Step 3.2: Send more event messages until a total of CFE_PLATFORM_EVS_LOG_MAX "
...
/$SC_$CPU_TST_EVS_SendEvtMsg INFO EventId="6" Iters=CFE_PLATFORM_EVS_LOG_MAX-evs_msg_sent_counter Milliseconds = 15
;;wait CFE_EVS_LOG_MAX-evs_msg_sent_counter+1
wait CFE_PLATFORM_EVS_LOG_MAX-evs_msg_sent_counter+1
...
write ";***********************************************************************"
;              Send 1<n<CFE_PLATFORM_EVS_LOG_MAX event messages
;              Verify that:
;                p@$SC_$CPU_EVS_LOGFULL = FALSE
;                $SC_$CPU_EVS_LOGOVERFLOWC = 0 
;                $SC_$CPU_EVS_LOGMODE = DISCARD (1)
...
write "; 	     CFE_PLATFORM_EVS_LOG_MAX"
write ";	     Requirement(s): cEVS3108_1, cEVS3018h"
write ";***********************************************************************"
;               Verify that:
;                 p@$SC_$CPU_EVS_LOGFULL = TRUE
...
/$SC_$CPU_TST_EVS_SendEvtMsg INFO EventId = "7" Iters = CFE_PLATFORM_EVS_LOG_MAX-evs_msg_sent_counter Milliseconds = 15
;;wait CFE_EVS_LOG_MAX-evs_msg_sent_counter+1
wait CFE_PLATFORM_EVS_LOG_MAX-evs_msg_sent_counter+1
...
write ";***********************************************************************"
;                 Log file dumped CFE_PLATFORM_EVS_LOG_MAX entries
...
;; Need to send TO command to add the SHORT telemetry packet so that these 
;; events can be verified
local stream = x'809'

/$SC_$CPU_TO_ADDPACKET Stream=stream Pkt_Size=x'0' Priority=x'0' Reliability=x'1' Buflimit=x'4'
wait 5

FOR i = 1 to 7 DO
  $SC_$CPU_EVS_SHORTEVENTID = 0
  ...
  ut_tlmwait $SC_$CPU_EVS_SHORTEVENTID, 6
  if (UT_TW_Status = UT_Success) then
    ;; Write the other fields of the short event packet
    write "Short Event App Name = '",$SC_$CPU_EVS_ShortAppName,"'"
    write "Short Event ID       = '",$SC_$CPU_EVS_ShortEventID,"'"
    write "Short Event Type     = '",p@$SC_$CPU_EVS_ShortEventType,"'"
    write "Short Event SCID     = '",$SC_$CPU_EVS_ShortSCID,"'"
    write "Short Event PID      = '",$SC_$CPU_EVS_ShortProcessorID,"'"
  else
    write "Short event Packet was not rcvd"
  endif

  ;; cFE 6.6.0 fixed the SHORT format to send a different tlm packet. 
  ;; Thus, Events will not be rcv'd. Need to rcv new pkt and print results
;;  IF ($SC_$CPU_find_event[1].num_found_messages = 1) THEN
;;    IF ($SC_$CPU_find_event[1].event_txt <> "") THEN
;;      write
...
#else
;
; _CFE_PLATFORM_EVS_LOG_ON_ configuration parameter is NOT defined.
;
write
