...
;	10/26/17	W. Moleski	Updated for cFE 6.6
;
;  Arguments
;	None 
;
...
ut_tlmwait $SC_$CPU_TBL_LASTUPDTBLNAME, "TST_TBL.dflt_tbl_02"
if (UT_TW_Status = UT_Success) then
...
write ";**********************************************************************"
write "; Step 6.1 : Verify cFE generates an error event message for an        "
write "; 	    invalid length command"
write ";**********************************************************************"
ut_setupevents "$SC", "$CPU", "CFE_TBL", CFE_TBL_LEN_ERR_EID, "ERROR", 1

errcnt = $SC_$CPU_TBL_CMDEC + 1
;; CPU1 is the default
rawcmd = "1804c000000200"
...
if (UT_TW_Status = UT_Success) then
  ...
  if ($SC_$CPU_find_event[1].num_found_messages = 1) then
    ...
    write "<!> Failed - Event Message not received for invalid length command."
  endif
else
...
write ";*********************************************************************"
write "; Step 9.2: Command a Power-On Reset on ",hostCPU
write ";*********************************************************************"
/$SC_$CPU_ES_POWERONRESET
wait 10
