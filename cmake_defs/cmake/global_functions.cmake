...
function(generate_config_includefile DESTFILE SUFFIX)
  ...
  foreach(SRC ${ARGN})
    ...
    if (INCL_INDX LESS 0)      
      ...
      if (EXISTS "${SRC_LOCAL_PATH}")
        ...
        message(STATUS "Adding header content\n\t${SRC_NATIVE}\n     to\n\t${DESTFILE}")
        set(INCL_FOUND 1)
      else()
  ...
  # Not fatal for export_symbols.inc which is expected to not exist.
  string(FIND ${DESTFILE} "export_symbols.inc" RES)
  if((${RES} EQUAL -1) AND (${INCL_FOUND} EQUAL 0))
     message(FATAL_ERROR "No valid content found for ${DESTFILE}")
  endif()
...
# FUNCTION: add_cpuid
#
# Adds the CPU id as a build target
#
##################################################################
function(add_tgt_cpuid CPUID)
  # If the list of CPUS is not explicitly specified, assume that
  # the CPUs form a sequential list, starting at CPU1.
  if (NOT DEFINED TGT_CPUIDS)
    set(TGT_CPUIDS "")
  endif()
  LIST(APPEND TGT_CPUIDS ${CPUID})
  SET(TGT_CPUIDS ${TGT_CPUIDS} PARENT_SCOPE)
endfunction(add_tgt_cpuid CPUID)

##################################################################
#
# FUNCTION: add_tgt_build
#
# Adds a build target
#
##################################################################
function(add_tgt_build CPUID)
  # If the list of CPUS is not explicitly specified, assume that
  # the CPUs form a sequential list, starting at CPU1.
  if (NOT DEFINED TGT_CPUIDS)
    set(TGT_CPUIDS "")
  ...
  LIST(APPEND TGT_CPUIDS ${CPUID})
  SET(TGT_CPUIDS ${TGT_CPUIDS} PARENT_SCOPE)
endfunction(add_tgt_build CPUID)
...
#
# If defined, this function uses the value of the list variable TGT_CPUIDS
# to specify the set of CPU ids to build. If this variable is not defined,
# we assume that the CPUs form a sequential list, starting at 1, with no gaps.
#
