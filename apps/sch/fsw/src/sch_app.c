...
int32 SCH_ProcessScheduleTable(void)
{
    ...
    CurrentSlot = SCH_AppData.NextSlotNumber;

    /* Update Minor Frame count */
    SCH_AppData.MinorFramesSinceTone = CurrentSlot;

    SCH_ProcessNextSlot();

    SCH_AppData.NextSlotNumber = CurrentSlot + 1;
    ...
    for (TableIndex = 0; TableIndex < SCH_MAX_MESSAGES; TableIndex++)
    {
        ...
        if (MessageID == SCH_UNUSED_MID)
        {
            ...
            */
            UnusedCount++;
            uint16 bufferIdx = 0;
            for(bufferIdx = 0; bufferIdx < SCH_MAX_MSG_WORDS*2; ++bufferIdx) {
               UserDataPtr = &MessageBuffer[bufferIdx];
               if (*UserDataPtr != 0)
               {
                 ...
                 (MessageID >= MinValue))
        {
            ...
            */
            ...
            else
            {
               ...
               uint8 TempBuffer[SCH_MAX_MSG_WORDS*2];
               if(PayloadLength > 0) {
