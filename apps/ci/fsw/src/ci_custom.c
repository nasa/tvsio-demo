...
void CI_CustomMain(void)
{
    ...
    while(size >= 0)
    {
        ...
        if (size > 0)
        {
        	OS_printf("CI received message...\n");
            /* Get Msg ID */
            msgId = CFE_SB_GetMsgId(sbMsg);
        	OS_printf("The message ID: 0x%x\n", msgId);
        	OS_printf("The CC: 0x%x\n", CFE_SB_GetCmdCode(sbMsg));
        	OS_printf("The message length: 0x%x\n", CFE_SB_GetTotalMsgLength(sbMsg));
            ...
            */
            ...
            {
            	OS_printf("0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x | 0x%02x 0x%02x\n",
            			sbMsg->Hdr.StreamId[0], sbMsg->Hdr.StreamId[1], sbMsg->Hdr.Sequence[0], sbMsg->Hdr.Sequence[1], sbMsg->Hdr.Length[0], sbMsg->Hdr.Length[1], sbMsg->SpacePacket.ApidQ.APIDQSubsystem[0], sbMsg->SpacePacket.ApidQ.APIDQSubsystem[1]);
