...
#define PACK_MID(mid) CFE_MAKE_BIG32(mid)
#else
#define PACK_MID(mid) CFE_MAKE_BIG16(mid)
#endif
 ...
 */
void SendLocalSubToPeer(int SubType, CFE_SB_MsgId_t MsgID,
...
void SBN_SendLocalSubsToPeer(SBN_PeerInterface_t *Peer)
{
    ...
    uint16 subCount = SBN.Hk.SubCount;

    int currIdx = 0;
    for (currIdx = 0; currIdx < SBN.Hk.SubCount; currIdx++)
    {
       PackSub(&SubMsg.Subs[currIdx], SBN.LocalSubs[currIdx].MsgID, SBN.LocalSubs[currIdx].Qos);
    }/* end for */
    ...
    for(NetIdx = 0; NetIdx < SBN.Hk.NetCount; NetIdx++)
    {
        ...
        if (Net != Peer->Net)
        {
           ...
           {
              SBN_PeerInterface_t *Peer2 = &Net->Peers[PeerIdx];
              int SubIdx = 0;
              for(SubIdx = 0; SubIdx < Peer2->Status.SubCount; ++SubIdx, ++currIdx){
                 PackSub(&SubMsg.Subs[currIdx], Peer2->Subs[SubIdx].MsgID,
                         Peer2->Subs[SubIdx].Qos);
              }
              subCount += Peer2->Status.SubCount;
           }
        }
    }

    SubMsg.SubCnt = CFE_MAKE_BIG16(subCount);
 ...
 */
void ProcessLocalSub(CFE_SB_MsgId_t MsgID, CFE_SB_Qos_t Qos)
{
    ...
    // If we are filtering all messages except the ones in the remap table, check that the local subscription
    // is in it
    if(SBN.RemapTable->RemapDefaultFlag == SBN_REMAP_DEFAULT_IGNORE) {
       int idx;
       for(idx = 0;idx < SBN.RemapCache.numEntries; ++idx) {
          if(SBN.RemapTable->Entry[idx].from == MsgID) {
             break;
          }
       }
       // The message ID was not found in the remap table, don't add as a local subscription.
       if(idx == SBN.RemapCache.numEntries) {
          return;
       }
    }
...
void ProcessSubFromPeer(SBN_PeerInterface_t *Peer, CFE_SB_MsgId_t MsgID,
    CFE_SB_Qos_t Qos)
{
    int idx = 0;
...
void SBN_ProcessSubsFromPeer(SBN_PeerInterface_t *Peer, void *Msg)
{
    ...
    uint16 SubCnt = CFE_MAKE_BIG16(SubMsg->SubCnt);
