...
#include <errno.h>
#include <limits.h>
#include <sys/resource.h>  /*Danny Strauss- for getrlimit() */
                      ...
                      uint32 flags)
{
    ...
    */
    ...
    /* Danny Strauss: remove geteuid() check to allow all users to set task attributes
    ** (priority, etc.) and replace it with a check of user's allowed system rt priority 
    ** limit
    */
    ...
    {
        printf("WARNING: Priority not set because you are limited to priority %d\n",
               (int)rl.rlim_cur);
    }
    else
    {
       printf("==== Setting %s priority %d\n", task_name, os_priority);
             ...
             */
             ...
             if ( Status == 0 )
             {
                ...
                *sem_id = possible_semid;
                ...
                sigfillset(&(OS_bin_sem_table[*sem_id].sigmask));
...
/*--------------------------------------------------------------------------------------
     Name: OS_BinSemSetSignalMask

    Purpose: Adds a signal ID to the specified Binary Semaphore to be allowed when
             calling OS_BinSemTake or OS_BinSemTimedWait calls.
    ...
    Returns: OS_SEM_FAILURE the semaphore was not previously  initialized or is not
             ...
             OS_SUCCESS if success
---------------------------------------------------------------------------------------*/
int32 OS_BinSemAddSignalMask( uint32 sem_id,
                              int sigid )
{
   ...
   if ( sigdelset( &( OS_bin_sem_table[sem_id].sigmask ), sigid ) == -1 )
   {
      return OS_SEM_FAILURE;
   }
   return OS_SUCCESS;
...
int32 OS_BinSemTake ( uint32 sem_id )
{
    ...
    ** deadlock.
    */
    if (pthread_sigmask(SIG_SETMASK, &(OS_bin_sem_table[sem_id].sigmask), &previous) != 0) {
        return(OS_SEM_FAILURE);
    } else if (pthread_mutex_lock(&(OS_bin_sem_table[sem_id].id)) != 0) {
        return(OS_SEM_FAILURE);
    }
    ...
    */
    if ( OS_bin_sem_table[sem_id].current_value < OS_bin_sem_table[sem_id].max_value )
    {
       ...
       */
       while ( OS_bin_sem_table[sem_id].current_value < OS_bin_sem_table[sem_id].max_value )
       {
...
int32 OS_BinSemTimedWait ( uint32 sem_id, uint32 msecs )
{
    ...
    */
    if ( OS_bin_sem_table[sem_id].current_value < OS_bin_sem_table[sem_id].max_value )
    {
       ...
       ** calls the pthread_cond_signal or pthread_cond_broadcast to aquire the mutex
       */
       while ( (ret != ETIMEDOUT) && (OS_bin_sem_table[sem_id].current_value < OS_bin_sem_table[sem_id].max_value) )
       {
             ...
             */
             ...
             if ( Status == 0 )
             {
                ...
                *sem_id = possible_semid;
                ...
                sigfillset(&(OS_count_sem_table[*sem_id].sigmask));
...
/*--------------------------------------------------------------------------------------
    ...
    Returns: OS_SEM_FAILURE the semaphore was not previously  initialized or is not
             ...
             OS_SUCCESS if success
---------------------------------------------------------------------------------------*/
int32 OS_CountSemAddSignalMask( uint32 sem_id,
                                int sigid )
{
   ...
   if ( sigdelset( &( OS_count_sem_table[sem_id].sigmask ), sigid ) == -1 )
   {
      return OS_SEM_FAILURE;
   }
   return OS_SUCCESS;
...
int32 OS_CountSemTake ( uint32 sem_id )
{
    ...
    ** deadlock.
    */
    if (pthread_sigmask(SIG_SETMASK, &(OS_count_sem_table[sem_id].sigmask), &previous) != 0) {
        return(OS_SEM_FAILURE);
    } else if (pthread_mutex_lock(&(OS_count_sem_table[sem_id].id)) != 0) {
        return(OS_SEM_FAILURE);
    }
    ...
    */
    if ( OS_count_sem_table[sem_id].current_value <= 0 )
    {
       ...
       ** calls the pthread_cond_signal or pthread_cond_broadcast to aquire the mutex
       */
       while ( OS_count_sem_table[sem_id].current_value < OS_count_sem_table[sem_id].max_value )
       {
...
int32 OS_CountSemTimedWait ( uint32 sem_id, uint32 msecs )
{
    ...
    */
    if ( OS_count_sem_table[sem_id].current_value <= 0 )
    {
       ...
       ** calls the pthread_cond_signal or pthread_cond_broadcast to aquire the mutex
       */
       while ( (ret != ETIMEDOUT) && (OS_count_sem_table[sem_id].current_value < OS_count_sem_table[sem_id].max_value) )
       {
