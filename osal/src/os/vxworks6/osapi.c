...
/*--------------------------------------------------------------------------------------
    ...
    Purpose: Adds a signal ID to the specified Binary Semaphore to be allowed when
             calling OS_BinSemTake or OS_BinSemTimedWait calls. (Not required for VxWorks)
 ...
 *          writing data to the UART
 ----------------------------------------------------------------------------*/ 
#ifdef OS_UTILITY_TASK_ON
#ifndef OS_LOG_TO_FILE
#define OS_LOG_TO_FILE 0x0
#endif
void UtilityTask()
{
  ...
  if(OS_LOG_TO_FILE != 0x0) {
     fp = fopen(OS_LOG_TO_FILE, "w");
  }
  ...
  OS_TaskDelay(3000);
   ...
   {    
       if(OS_DroppedMessages > 0)
       {
          #ifdef OS_DEBUG_PRINTF
             if(OS_LOG_TO_FILE != 0x0) {
                // Print to file and flush to make sure we don't lose any messages from the end of the file.
                fprintf(fp, "Dropped Msgs to UART = %d\n",OS_DroppedMessages);
                fflush(fp);
             } else {
